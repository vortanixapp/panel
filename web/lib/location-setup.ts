...
// Список объявлен на уровне модуля и вычисляется один раз при загрузке,
// поэтому подписи хранятся ключами: готовый текст застыл бы на том языке,
// который стоял в момент загрузки страницы.
...
// t приходит параметром, а не берётся из модуля: перевод должен считываться
// в момент отрисовки, иначе подпись не сменится при переключении языка.
