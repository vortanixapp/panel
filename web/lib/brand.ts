...
// Палитра лендинга: ключи те же, что в форме оформления (primary, background,
// card, border и прочие).
export function getRuntimeTemplateColors(): Record<string, string> {
  return cachedBranding?.colors ?? {};
}

// Блоки лендинга: ключ выключен, только если владелец панели явно снял его.
// Отсутствие ключа значит «показывать», иначе пустая настройка прятала бы всё.
