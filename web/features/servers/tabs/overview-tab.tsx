...
// Сколько копий показывать в обзоре. Полный список — на вкладке «Копии»;
// здесь важно ответить на один вопрос: копии вообще делаются?
...
export function ServerOverviewTab() {
  ...
  // Каталог копий отдаётся вперемешку с подкаталогами; берём только файлы.
  // Порядок API не обещает, поэтому сортируем по имени: в него входит время
  // создания, и свежие оказываются сверху.
    ...
    <div className="flex flex-col gap-[18px]">
      ...
      <div className="grid grid-cols-1 items-start gap-[18px] lg:grid-cols-[minmax(0,1fr)_minmax(0,1.35fr)]">
        ...
        <div className="flex flex-col gap-[18px]">
          ...
          {/* Раньше здесь стояли четыре ссылки на вкладки «Настройки», «Логи»,
              «Firewall» и «Планировщик» — те самые, что и так стоят в панели
              вкладок над страницей, да ещё подписанные именами файлов и
              утилит. Навигация вместо сведений. Теперь панель отвечает на
              вопрос, ответа на который на этой странице нигде не было:
              делаются ли резервные копии и когда последняя. */}
          <Panel
