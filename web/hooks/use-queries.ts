...
/**
 * Состояния, из которых сервер вот-вот перейдёт в другое.
 *
 * Пока сервер в одном из них, десятисекундная пауза между опросами читается как
 * зависание: человек нажал кнопку, увидел «Запускается…» и дальше ничего не
 * происходит. Живые события приходят по WebSocket, но соединение может быть
 * недоступно, поэтому опрос остаётся страховкой — просто заметно более частой.
 */
...
export function patchServerStatus(
  ...
  // Страница сервера рисуется из serverDetail, и без этой строки живые события
  // до неё не доходили вовсе: статус менялся один раз, когда возвращался запрос
  // питания, и дальше замирал до следующего опроса — отсюда и ощущение рывка.
...
/**
 * Обновляет шкалу установки по живому событию.
 *
 * Опрос лога установки остаётся — он приносит строки вывода, — но шкала теперь
 * не ждёт следующего запроса: событие приходит от агента через relay почти
 * сразу, и полоса движется плавно, а не ступенями по две секунды.
 */
    ...
    old
      ? {
          ...
          progress: {
            ...
            // Значение измеренное, а не синтезированное: шкала должна
            // показывать реальный процент, а не бесконечную пульсацию.
            derived: false,
          },
        }
      : old
