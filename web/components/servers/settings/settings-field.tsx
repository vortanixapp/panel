...
/**
 * Одно поле настроек. Вид контрола определяет схема с бэкенда, а не список на
 * фронте: раньше булевы значения показывались выпадающим списком с вариантами
 * «true» и «false», потому что типов у полей просто не было.
 */
  ...
  return (
    <label className="grid gap-1.5">
      <span className={cn("flex flex-wrap items-center gap-2 text-[11.5px]", "text-[var(--vx-muted)]")}>
        ...
        {/* Бейдж только там, где перезапуск действительно нужен: если повесить
            его на каждое поле, он перестанет что-либо значить. */}
...
function Control({
  ...
  if (field.kind === "enum" && field.options?.length) {
    return (
      ...
      >
        {/* Значение из файла может не входить в список — например конфиг правили
            руками. Молча подменять его первым вариантом нельзя: сохранение
            переписало бы то, чего пользователь не трогал. */}
    ...
    return (
      <span className="relative flex">
        <input
          ...
          // Настоящее значение сюда не приходит никогда. Пустое поле означает
          // «не менять» — так пароль нельзя ни подсмотреть, ни случайно стереть,
          // открыв и сохранив форму.
