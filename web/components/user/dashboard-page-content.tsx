...
// Шаг «что сделать дальше»: подписи храним ключами, текст собирается
// на отрисовке — иначе язык застыл бы на моменте расчёта useMemo.
type NextStep = {
  href: string;
  titleKey: string;
...
export function DashboardPageContent() {
  ...
  // без него главная не перерисуется при смене языка.
  useT();
  ...
  // Шаги считаются в useMemo, поэтому храним ключи, а не готовый текст:
  // иначе подписи застыли бы на языке, который стоял при первом расчёте.
