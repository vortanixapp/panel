  ...
  return (
    <div className="flex flex-col gap-4 rounded-2xl border border-border bg-card px-5 py-[18px] transition-colors hover:border-[var(--vx-panel-hover-line)]">
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(190px,1fr))] gap-[14px]">
        {/* Занятое место сервер не отдаёт — показываем размер тарифа без доли.
            Нарисовать здесь полосу значило бы выдать выдумку за измерение. */}
...
export function MyHostingPageContent() {
  // Карточка аккаунта зовёт t() напрямую, здесь хук нужен ради подписки:
  ...
  return (
    <PageShell variant="user">
      <div className="flex w-full flex-col gap-[22px]">
        ...
        {/* Приглашение взять ещё один сайт. Для пустого списка это же место
            работает как единственный призыв — отдельный экран «пусто» показывал
