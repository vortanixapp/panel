...
// Набор блоков лендинга задаётся в админке («Блоки лендинга»), но раньше
// настройка никуда не доезжала и лендинг всегда показывал всё. Ключ, которого
// в настройке нет, считаем включённым: пустая настройка не должна прятать
// страницу целиком.
...
// Цвета лендинга из админки ложатся на те же токены, на которых построена сама
// страница: раньше форма их сохраняла, а лендинг всегда рисовался стандартной
// палитрой. Ключи формы совпадают с именами токенов, кроме accent — у него в
// теме есть ещё и цвет текста поверх.
...
export function LandingBody() {
  ...
  // Тексты берём через геттеры, а не из констант модуля: константа собралась бы
  // один раз на том языке, который стоял при загрузке страницы.
  ...
  return (
    <div ref={rootRef} style={landingColorStyle(templateColors)}>
      ...
      <section className="relative overflow-hidden border-b border-border px-4 pt-16 pb-16 sm:px-10 sm:pt-[88px] sm:pb-[72px]">
        ...
        <div className="landing-glow pointer-events-none absolute -top-[300px] left-1/2 h-[600px] w-[1100px] max-w-none -translate-x-1/2 rounded-full bg-white/10 blur-[110px]" />
        ...
        <div className="relative mx-auto grid max-w-[1240px] items-center gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:gap-14">
          <div>
            ...
            >
              <Link
                href="/register"
                className="vx-btn vx-sheen relative overflow-hidden rounded-[10px] px-[26px] py-[15px] text-[15px] font-semibold shadow-[0_0_44px_rgba(255,255,255,0.14)]"
              >
            ...
            >
              ...
              <span className="size-1 rounded-full bg-[#333436]" />
          ...
          <div className="landing-float">
            <div className="overflow-hidden rounded-[14px] border border-[var(--vx-line-strong)] bg-[var(--vx-surface-3)] shadow-[0_40px_90px_rgba(0,0,0,0.6),0_0_0_1px_rgba(255,255,255,0.04)]">
      ...
      >
        <div className="mx-auto max-w-[1180px]">
          <div className="flex flex-wrap items-baseline justify-between gap-6">
            ...
            <Link
              href="/games"
              className="text-[13.5px] whitespace-nowrap text-primary hover:text-white"
            >
          ...
          <div className="mt-4 grid grid-cols-1 gap-px overflow-hidden rounded-xl border border-border bg-border sm:grid-cols-2 lg:grid-cols-3">
            {LANDING_MORE_GAMES.map((g) => (
              <Link
                key={g.name}
                href="/games"
                className="flex items-center gap-3.5 bg-[var(--vx-surface)] px-5 py-[18px] transition-colors hover:bg-[#101113]"
              >
        ...
        <div className="mx-auto max-w-[1180px]">
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {steps.map((s, i) => (
              ...
              >
                <div className="font-mono text-[52px] leading-none font-bold tracking-[-0.04em] text-[#1f2022]">
                  {s.n}
                </div>
      ...
      >
        <div className="mx-auto max-w-[1180px]">
          ...
          <div className="mt-9 grid auto-rows-[minmax(150px,auto)] grid-cols-1 gap-3.5 sm:grid-cols-3">
            <div
              data-reveal
              className="vx-reveal relative overflow-hidden rounded-[14px] border border-[var(--vx-line-strong)] bg-[linear-gradient(150deg,#101113,#090A0C)] p-[30px] sm:col-span-2 sm:row-span-2"
            >
        ...
        <div className="mx-auto grid max-w-[1180px] items-center gap-14 lg:grid-cols-[0.85fr_1.15fr]">
          <div>
            ...
            <div className="mt-[26px] flex flex-col gap-3">
              {panelPoints.map((p) => (
                <div key={p} className="flex gap-3 text-sm leading-[1.5] text-[#b8b9bb]">
          ...
          <div data-reveal className="vx-reveal flex flex-col gap-3.5">
            <Link
              href="/dashboard"
              className="block overflow-hidden rounded-[14px] border border-[var(--vx-line-strong)] bg-[var(--vx-surface-3)] shadow-[0_30px_70px_rgba(0,0,0,0.5)]"
            >
            ...
            <div className="relative h-[230px] overflow-hidden rounded-[14px] border border-border bg-[var(--vx-surface-4)]">
              ...
              <div className="pointer-events-none absolute inset-x-0 bottom-0 h-20 bg-[linear-gradient(180deg,rgba(10,11,13,0),rgba(10,11,13,0.92))]" />
            </div>
          </div>
        </div>
      </section>
            ...
            {pricing.map((p, i) => (
              <div
                ...
                className={cn(
                  ...
                  p.highlighted
                    ? "border-[var(--vx-btn-line)] bg-[linear-gradient(150deg,#141517,#0B0C0E)]"
              ...
              >
                ...
                <div className="mt-[22px] flex flex-col gap-2.5">
                  {p.specs.map((s) => (
                    <div key={s} className="flex gap-2.5 text-[13.5px] text-[#b8b9bb]">
                      <span className="text-primary">/</span>
                      <span>{s}</span>
                    </div>
                  ))}
      ...
      <section className="px-4 py-16 sm:px-10 sm:pt-20 sm:pb-25">
        <div
          data-reveal
          className="vx-reveal relative mx-auto flex max-w-[1180px] flex-col items-start gap-8 overflow-hidden rounded-2xl border border-[var(--vx-hairline)] bg-[linear-gradient(140deg,#101113,#0A0B0D)] px-7 py-12 sm:flex-row sm:items-center sm:justify-between sm:px-12 sm:py-14"
        >
          <div className="landing-glow pointer-events-none absolute -top-[160px] -right-[80px] size-[420px] rounded-full bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.08),rgba(255,255,255,0)_70%)]" />
