...
/**
 * Строки переключения аккаунтов для выпадающего меню.
 *
 * Живёт отдельным компонентом, потому что таких меню в панели два: в подвале
 * бокового меню (`nav-user`) и в шапке страницы (`profile-dropdown`). Список
 * аккаунтов в них обязан совпадать — человек не должен гадать, из какого угла
 * переключаться.
 *
 * Вставляется внутрь `DropdownMenuContent`.
 */
export function AccountSwitchItems() {
  ...
  // Список читается только в браузере: на сервере localStorage нет, и без
  // задержки до монтирования серверная разметка разошлась бы с клиентской.
  ...
  useEffect(() => {
    reload();
    // Соседняя вкладка могла добавить аккаунт или выйти из него. Событие
    // storage приходит только «чужим» вкладкам — своих правок мы так не увидим,
    // но после них страница и так перезагружается.
  ...
  async function switchTo(account: StoredAccount) {
    ...
    if (outcome.ok) {
      // Кэш чистим до ухода со страницы: перезагрузка и так его потеряет, но
      // между этими двумя строками успевает отрисоваться кадр, и чужих данных
        ...
        <>
          ...
          {others.map((account) => (
            <DropdownMenuItem
              key={account.id}
              disabled={!!busy}
              // Меню не закрываем: переключение может закончиться просьбой
              // ввести пароль, и сказать об этом нужно до ухода со страницы.
              onSelect={(e) => {
                e.preventDefault();
...
/** Галочка активного аккаунта — та же разметка, что у переключателя темы. */
