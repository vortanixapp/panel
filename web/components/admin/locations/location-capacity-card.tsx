...
const GAMES_SHOWN = 12;

export function fmtMB(t: TranslateFn, mb: number): string {
  if (!mb) return "0";
  if (Math.abs(mb) >= 1024) {
    const gb = (mb / 1024).toLocaleString(localeTag(), { maximumFractionDigits: 1 });
    return `${gb} ${t("admin.infra.unit_gb")}`;
  }
...
export function LocationCapacityCard({ locationId }: { locationId: string }) {
  ...
  const hiddenGames = Math.max(0, data.games.length - GAMES_SHOWN);
    ...
    <section className="flex flex-col gap-5 rounded-2xl border bg-card px-5 py-5 sm:px-6">
      ...
      {!data.metrics_known ? (
        ...
        <div className="grid grid-cols-2 gap-2.5 md:grid-cols-3">
          <MiniStat label={t("admin.capacity.servers_on_node")}>
            {data.servers}
            {data.max_servers !== null ? ` / ${data.max_servers}` : ""}
          </MiniStat>
          <MiniStat label={t("admin.capacity.ram_on_node")}>
            {fmtMB(t, data.node_ram_mb)}
          </MiniStat>
          <MiniStat label={t("admin.capacity.allocated_ram")}>
          ...
          <MiniStat label={t("admin.capacity.free_disk")}>
            {fmtMB(t, data.free_disk_mb)}
          </MiniStat>
          <MiniStat label={t("admin.capacity.allocated_cpu")}>
      ...
      {data.games.length > 0 && (
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {data.games.slice(0, GAMES_SHOWN).map((g) => (
              <span
                key={g.slug}
                className={
                  g.fits > 0
              ...
              >
                {g.name}: <b className="tabular-nums">{g.fits}</b>
              </span>
            ))}
            {hiddenGames > 0 && (
      ...
      <div className="flex flex-col gap-3 border-t pt-4">
        <span className="text-[13px] font-medium">
          {t("admin.capacity.settings_title")}
        </span>
...
function MiniStat({
  ...
  return (
    <div className="flex min-w-0 flex-col gap-1 rounded-lg border bg-muted/20 px-3 py-2.5">
      <span className="truncate text-[11px] text-muted-foreground">{label}</span>
      <span
        className={
          danger
            ? "truncate font-mono text-sm text-destructive tabular-nums"
