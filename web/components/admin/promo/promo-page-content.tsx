...
// Поле datetime-local ждёт «ГГГГ-ММ-ДДTЧЧ:ММ». Раньше здесь обрезалось до даты,
// и акцию «до 18:00 сегодня» настроить было нельзя.
...
function toPayload(f: FormState): AdminPromotionInput {
  return {
    // Безкодовая акция всё равно должна как-то называться в списке.
...
export function PromoPageContent() {
  ...
  function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    // Пустой код разрешён: это автоматическая акция, она подбирается сама.
    // Раньше форма его требовала, хотя подзаголовок страницы обещал обратное.
