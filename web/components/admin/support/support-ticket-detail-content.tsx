...
/** Заготовки ответа.
 *
 * Оператор пишет одно и то же по десять раз на смену. Заготовка не отправляет
 * ответ, а подставляет его в поле: текст почти всегда нужно дописать под
 * обращение, и отправка одним нажатием приводила бы к ответам не по делу.
 *
 * Список читается на уровне модуля, поэтому храним ключи: готовый текст застыл
 * бы на языке, который стоял в момент загрузки страницы. */
const CANNED = [
  {
...
// Ключи, а не подписи — по той же причине, что и у заготовок ответа.
const STATUSES = [
...
function MessageBubble({
  ...
  // У оператора свои сообщения справа, клиентские слева — зеркально
  // клиентскому экрану. Иначе, переключаясь между ролями, читаешь переписку
  // наоборот и путаешь, кто что написал.
  return (
...
export function AdminSupportTicketDetailContent() {
  ...
  /** Ответить и сразу закрыть. Порядок важен: сначала ответ, потом закрытие —
   *  иначе клиент получает закрытое обращение без объяснения. */
  async function replyAndClose() {
    ...
    try {
      ...
      // Об ошибке уже сказали мутации, второй раз не сообщаем.
    }
  }
        ...
        <div className="grid grid-cols-1 items-start gap-5 lg:grid-cols-[minmax(0,1fr)_300px]">
          ...
          <div className="flex flex-col gap-[14px] lg:sticky lg:top-[88px]">
            ...
            <div className="flex flex-col gap-[14px] rounded-2xl border border-border bg-card px-5 py-4">
              ...
              {/* Назначение — только на себя: список операторов сервер не
                  отдаёт, а селект с одним пунктом «я» ничем не лучше кнопки. */}
              {ticket?.assigned_admin_email ? (
