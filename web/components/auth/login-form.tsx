...
export function LoginForm() {
  ...
  // Добавление ещё одного аккаунта к уже открытой сессии, а не вход заново.
  ...
  // Пришедшего с живой сессией на форму входа пускаем внутрь: иначе он видит
  // предложение войти, уже будучи вошедшим. Исключение — добавление аккаунта:
  // туда приходят именно с живой сессией, и этот же переход выбрасывал бы
  // обратно на дашборд, не дав ввести вторую почту.
  useEffect(() => {
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      ...
      // Двухфакторная проверка: токенов ещё нет, есть только пропуск к
      // следующему шагу. Раньше эта ветка отсутствовала, и ответ уходил дальше
      // как успешный вход — в хранилище оседала строка "undefined", а человек
      // попадал в панель с нерабочим токеном.
      ...
      // Кэш от прежнего входа выбрасываем до перехода: иначе первые экраны
      // отрисуются чужими данными, пока не придут свои.
      queryClient.clear();
