...
const (
	// writeWait ограничивает одну запись. Без него зависшая отправка в
	// полуоткрытое соединение держала бы writeMu вечно: агент замолкал
	// целиком — ни heartbeat, ни метрик, — но продолжал числиться
	// подключённым.
	writeWait = 15 * time.Second

	// pongWait — сколько ждём хоть чего-нибудь от relay. Тот шлёт ping раз в
	// 25 секунд, так что молчание дольше этого означает оборванную связь.
	// Без дедлайна чтения агент вечно ждал бы данных из мёртвого сокета: TCP
	// об обрыве не сообщает, и переподключение не наступало никогда.
	pongWait = 90 * time.Second

	// handshakeTimeout: у DefaultDialer таймаут не задан вовсе, и подвисшее
	// рукопожатие остановило бы агента навсегда.
...
func (a *Agent) send(payload []byte) error {
	...
	if err != nil {
		// После неудачной записи gorilla помечает соединение сломанным
		// навсегда, и дальше по нему не пройдёт ничего. Закрываем сразу:
		// читающий цикл разблокируется, и Run поднимет новый сеанс. Иначе
		// агент остаётся подключённым, но немым — снаружи он неотличим от
		// исправного, а нода тихо уходит в офлайн.
...
func (a *Agent) Run() {
	...
	// Пауза растёт, пока relay недоступен, и сбрасывается, как только сеанс
	// состоялся: после короткого сбоя связь восстанавливается почти сразу, а
	// лежачий relay не забрасывается попытками каждые пять секунд.
...
func (a *Agent) connect() error {
	...
	// Дедлайн чтения продлевается пингом relay и любым его сообщением. Ставим
	// до первой записи: если сокет уже мёртв, узнаем об этом, а не зависнем.
	//
	// Продлевать нужно именно на ping: пингует relay, а не мы. Стандартный
	// обработчик gorilla отвечает pong'ом, но дедлайн не трогает — с одним
	// лишь PongHandler связь рвалась ровно через pongWait, хотя была живой.
...
func (a *Agent) loop() {
	...
	// Каналом эти два цикла останавливаются вместе с соединением. Раньше они
	// запускались заново на каждом переподключении и не завершались никогда,
	// так что горутины копились, а heartbeat уходил пачками.
...
func (a *Agent) heartbeat(done <-chan struct{}) {
	...
	for {
		...
		// send сам закроет связь при ошибке — здесь просто выходим, чтобы не
		// молотить в заведомо мёртвое соединение до переподключения.
...
func (a *Agent) handleCommand(data []byte) {
	...
	case "power":
		...
		switch action {
		case "start", "restart":
			// Параметры кладём до запуска: entrypoint читает файл один раз, когда
			// поднимает процесс, и запись после docker start применилась бы только
			// к следующему разу.
			//
			// Ошибку не проглатываем и сервер не поднимаем. У части игр (Valheim,
			// ARK) в этой строке лежит имя мира: старт без неё создаст новый мир
			// рядом со старым, и клиент решит, что потерял сохранение.
		...
		return
	case "update":
		// Обновление игры панель отправляла с самого начала, а ветки для него
		// здесь не было: команда падала в конец switch, агент отвечал «готово»
		// и слал статус по умолчанию — «остановлен». Панель показывала
		// «обновляется», на ноде не происходило ничего, а работавший сервер
		// оказывался в базе остановленным.
		...
		primaryPort := docker.IntFromPayload(cmd.Payload["primary_port"])
		bindIP, _ := cmd.Payload["bind_ip"].(string)
		// Обновление идёт минутами, поэтому отвечаем сразу и работаем в фоне:
		// иначе команда упирается в таймаут ожидания ответа.
		//
		// Статус именно «updating», а не «installing»: последний означает первую
		// установку и сбрасывает состояние установки сервера на «идёт», после
		// чего по завершении владельцу приходит ложное «сервер готов».
	...
	case "destroy":
		...
		// Цепочка iptables и файл расписания переживали удаление контейнера.
		// Первое опаснее: docker переиспользует адреса, и правила удалённого
		// клиента начинали резать трафик серверу другого клиента.
	...
	case "archive_cache_fetch":
		// Команда уровня ноды, а не сервера: архив кладётся в общий кэш
		// локации и потом используется всеми её серверами.
	...
	case protocol.ActionPortsSync:
		// Публикация портов задаётся при создании контейнера, поменять её у
		// работающего нельзя. Поэтому сохраняем список и, если он изменился,
		// пересоздаём контейнер — только тогда порт действительно открывается.
...
func (a *Agent) installAndStart(serverID, name, gameID string, limits map[string]any, dockerImage string, primaryPort int, bindIP string, spec docker.InstallSpec) {
	...
	// SteamCMD отработал от root и оставил файлы за собой — возвращаем их
	// владельцу SFTP-доступа, иначе клиент не сможет править свои конфиги.
...
// updateAndStart обновляет файлы игры и возвращает сервер в прежнее состояние.
//
// Обновление на живом контейнере невозможно: SteamCMD пишет в те же файлы,
// которые держит запущенный сервер. Поэтому останавливаем, обновляем и
// запускаем обратно — но только если сервер работал до обновления.
func (a *Agent) updateAndStart(serverID, name, gameID string, limits map[string]any, dockerImage string, primaryPort int, bindIP string, spec docker.InstallSpec) {
	...
	// SteamCMD работает от root: без возврата владельца клиент перестанет
	// править собственные конфиги по SFTP.
	docker.ReapplyOwnership(ctx, serverID)
