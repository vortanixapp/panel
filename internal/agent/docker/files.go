...
func ListFiles(ctx context.Context, serverID, dir string) ([]FileEntry, error) {
	...
	// Список тоже читаем с хоста: на остановленном сервере docker exec падает, и
	// файловый менеджер показывал пустоту вместо содержимого. Разбор вывода
	// `ls -la` заодно перестаёт быть нужен — имена с пробелами он ломал.
