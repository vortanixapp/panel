...
func CollectStatsExtended(ctx context.Context, serverID string, diskLimitMB int) (Stats, error) {
	...
	// Где есть квота, расход берём у неё: ядро уже ведёт счёт, а обход дерева
	// на сервере с сотней тысяч файлов занимает секунды и бьёт по диску на
	// каждый опрос статистики. Ноль означает «квоты нет» — считаем как прежде.
