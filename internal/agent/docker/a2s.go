...
// isMcBedrockGame — Minecraft Bedrock.
//
// В каталоге он делит значение QueryProto = mc с пятью изданиями Java, хотя
// протокол опроса у него совсем другой: UDP RakNet против TCP у Java.
// Различаем по образу — это единственное, чем они отличаются в каталоге.
...
// sampInfo — ответ SA-MP на запрос 'i'.
//
// Кроме счётчиков в пакете есть режим игры: у SA-MP карта одна на всю игру
// (Сан-Андреас), и «режим» — единственное, что имеет смысл показывать в её
// строке. Берём его из пакета, а не из server.cfg: в конфиге лежит имя
// .amx-скрипта вроде «grandlarc», а сервер сообщает то, что скрипт выставил
// через SetGameModeText — человекочитаемое «Role Play» или «Derby».
...
// sampString читает строку с 4-байтовой длиной впереди.
func sampString(data []byte, off int) (string, int) {
	...
	off += 4
	// Длина приходит по сети: без верхней границы битый или враждебный ответ
	// увёл бы срез за пределы буфера.
...
// querySampFull — тот же запрос 'i', но с разбором строковой части пакета.
func querySampFull(host string, port int) (*sampInfo, error) {
	...
	// 11 байт заголовка (SAMP + ip + порт + код), затем байт пароля.
	off := 12
	out := &sampInfo{
