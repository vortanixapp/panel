...
// passwords — чем пробовать войти: сначала актуальный пароль, следом прежний
// вычисляемый.
//
// Пароль root теперь случайный и лежит на ноде, но меняет его шаг установки
// «MySQL», а увидеть новый агент может только после того, как его контейнер
// пересоздадут с примонтированным /opt/vortanix — то есть после шага «Vortanix
// Agent». Между этими двумя шагами агент остался бы без доступа к базам, и все
// операции с ними у клиентов отваливались бы. Запасной пароль убирает эту
// зависимость от порядка шагов.
...
func mysqlRun(ctx context.Context, inst mysqlInstance, args ...string) ([]byte, error) {
	...
	for _, pw := range inst.passwords() {
		...
		// Не пароль — повторять смысла нет: контейнер не поднят, SQL с ошибкой
		// и прочее вернутся тем же результатом.
