...
var defaults = []Target{
	{Key: "api", Name: "Panel API", Path: "/health", Public: true},
	{Key: "relay", Name: "Node link", Path: "/health", Public: true},
	{Key: "console", Name: "Server console", Path: "/health", Public: true},
	{Key: "panel", Name: "Web interface", Path: "/", Public: true},
}

func LoadTargets() []Target {
	out := make([]Target, 0, len(defaults))
	for _, t := range defaults {
		raw := strings.TrimSpace(os.Getenv("STATUS_" + strings.ToUpper(t.Key) + "_URL"))
		if raw == "" {
			continue
		}
