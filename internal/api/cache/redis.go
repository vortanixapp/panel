...
// InvalidateTenantServers снимает список серверов у всех пользователей
// арендатора. Ключ стал пользовательским: список зависит от того, кто смотрит,
// а под общим ключом ответ администратора доставался обычному клиенту.
func (c *Cache) InvalidateTenantServers(ctx context.Context, tenantID string) error {
	var cursor uint64
	for {
		keys, next, err := c.rdb.Scan(ctx, cursor, "t:"+tenantID+":servers:*", 200).Result()
		if err != nil {
			return err
		}
		if len(keys) > 0 {
...
func (c *Cache) InvalidateTenantNodes(ctx context.Context, tenantID string) error {
	return c.rdb.Del(ctx, "t:"+tenantID+":nodes").Err()
}
