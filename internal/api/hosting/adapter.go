...
// apiSnippet обрезает тело ответа панели для сообщения об ошибке: на 500-й
// панели отвечают целой HTML-страницей, и она бы уехала в интерфейс хостера
// как есть. Режем по рунам — иначе на кириллице получился бы битый хвост.
...
func NewAdapter(cfg ServerConfig) PanelAdapter {
	// Неполные доступы — общий случай для всех панелей: пока хостер не вписал
	// api_url/логин/токен, честнее вернуть заглушку с внятной причиной, чем
	// уронить операцию на пустом URL.
