...
func (h *Handler) GetBranding(w http.ResponseWriter, r *http.Request) {
	...
	// Оформление задавалось в админке целиком, а до панели доезжал один цвет:
	// палитра лендинга, набор блоков и вариант меню никуда не отдавались и
	// поэтому ни на что не влияли.
...
// jsonObjectOfStrings — цветовые токены оформления. Настройка хранится строкой,
// которую владелец панели правит руками, поэтому мусор молча отбрасываем:
// сломанный JSON не должен ронять брендинг всей панели.
...
// jsonObjectOfBools — включённые блоки лендинга.
func jsonObjectOfBools(raw string) map[string]bool {
	...
	for k, v := range parsed {
		...
		case bool:
			out[k] = val
		case string:
			// Значения правят руками, и «false» строкой встречается чаще, чем
			// хотелось бы: считать её включённым блоком было бы неожиданно.
