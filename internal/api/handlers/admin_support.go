...
func (h *Handler) AdminSupportUpdate(w http.ResponseWriter, r *http.Request) {
	...
	tag, err := h.dbOf(r.Context()).Exec(r.Context(), `
		UPDATE core.support_tickets SET
			...
			assigned_admin_id = CASE WHEN $4::bool THEN $5::uuid ELSE assigned_admin_id END,
			closed_at         = CASE WHEN $2 = 'closed' THEN now()
			                         WHEN $2 IS NOT NULL THEN NULL
			                         ELSE closed_at END,
			updated_at        = now()
		WHERE id = $1
	`, id, body.Status, body.Priority, assignChanged, assignee)
	if err != nil {
	...
	audit(r.Context(), h.dbOf(r.Context()), claims.UserID, "support.update", "ticket:"+id,
