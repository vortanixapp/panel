...
func (h *Handler) signCatalogArchive(kind, id string) (string, error) {
...
func (h *Handler) deliverCatalogArchive(r *http.Request, nodeID, kind, itemID string) (string, error) {
	table := "core.plugins"
	if kind == catalogMaps {
		table = "core.maps"
	}
	...
	err := h.dbOf(r.Context()).QueryRow(r.Context(),
		//nolint:gosec
		"SELECT COALESCE(archive_path, ''), COALESCE(archive_size, 0) FROM "+table+" WHERE id = $1::uuid",
		itemID).Scan(&rel, &size)
	if err != nil {
	...
	token, err := h.signCatalogArchive(kind, itemID)
	if err != nil {
		return "", err
	}
	...
	_, _ = h.dbOf(r.Context()).Exec(r.Context(),
		//nolint:gosec
		"UPDATE "+table+" SET archive_location_id = $2::uuid WHERE id = $1::uuid",
		itemID, nodeID)
