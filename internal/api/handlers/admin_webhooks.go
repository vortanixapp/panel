...
func (h *Handler) AdminWebhookCreate(w http.ResponseWriter, r *http.Request) {
	...
	if h.dbOf(r.Context()).QueryRow(r.Context(), `
		INSERT INTO core.webhooks ( url, events, secret, description)
		VALUES ( $1, $2::jsonb, $3, $4)
		RETURNING id::text
	`, target, eventsJSON, secret, strings.TrimSpace(body.Description)).Scan(&id) != nil {
		writeError(w, http.StatusInternalServerError, "database error")
		return
	}
	audit(r.Context(), h.dbOf(r.Context()), claims.UserID, "webhook.create", "webhook:"+id,
		map[string]any{"url": target, "events": events})
	h.auditAlert(r.Context(), claims.UserID, claims.Email, "webhook.create", target)
...
func (h *Handler) AdminWebhookUpdate(w http.ResponseWriter, r *http.Request) {
	...
	tag, err := h.dbOf(r.Context()).Exec(r.Context(), `
		...
		WHERE id = $1
	`, id, body.Active, eventsJSON, strings.TrimSpace(body.Description))
	if err != nil {
		writeError(w, http.StatusInternalServerError, "database error")
		return
	}
...
func (h *Handler) AdminWebhookDelete(w http.ResponseWriter, r *http.Request) {
	...
	id := chi.URLParam(r, "id")
	tag, err := h.dbOf(r.Context()).Exec(r.Context(), `DELETE FROM core.webhooks WHERE id = $1`,
		id)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "database error")
		return
...
func (h *Handler) AdminWebhookTest(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := h.dbOf(r.Context()).Exec(r.Context(), `
		...
		VALUES ( $1, 'webhook.test', $2::jsonb, now())
	`, id, payload); err != nil {
		writeError(w, http.StatusInternalServerError, "database error")
		return
	}
...
func (h *Handler) emitWebhook(ctx context.Context, event string, payload map[string]any) {
