...
func (h *Handler) ServerMysqlResetPassword(w http.ResponseWriter, r *http.Request) {
	...
	_, err := h.dbOf(r.Context()).Exec(r.Context(), `
		UPDATE core.servers SET config = jsonb_set(COALESCE(config, '{}'::jsonb), '{mysql}', $2::jsonb)
		WHERE id = $1
	`, serverID, raw)
	if err != nil {
	...
	if _, ok := h.agentCommandForServer(w, r, serverID, "mysql_create_db", h.mysqlAgentPayload(r, serverID, map[string]any{
...
func (h *Handler) ServerMysqlDelete(w http.ResponseWriter, r *http.Request) {
	...
	serverID := chi.URLParam(r, "id")
	info := h.loadServerMysql(r, serverID)
	if _, ok := h.agentCommandForServer(w, r, serverID, "mysql_delete_db", h.mysqlAgentPayload(r, serverID, info)); !ok {
		return
	}
	_, err := h.dbOf(r.Context()).Exec(r.Context(), `
		UPDATE core.servers SET config = config - 'mysql' WHERE id = $1
	`, serverID)
	if err != nil {
...
func (h *Handler) mysqlAgentPayload(r *http.Request, serverID string, base map[string]any) map[string]any {
...
func (h *Handler) loadServerMysql(r *http.Request, serverID string) map[string]any {
	var configRaw []byte
	var nodeMeta []byte
	_ = h.dbOf(r.Context()).QueryRow(r.Context(), `
		...
		WHERE s.id = $1
	`, serverID).Scan(&configRaw, &nodeMeta)
