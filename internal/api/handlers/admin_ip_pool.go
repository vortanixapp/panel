...
func (h *Handler) AdminNodeIPAdd(w http.ResponseWriter, r *http.Request) {
	...
	var exists bool
	if h.dbOf(ctx).QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM core.nodes WHERE id = $1)`,
		nodeID).Scan(&exists) != nil || !exists {
	...
	for _, addr := range addresses {
		tag, err := h.dbOf(ctx).Exec(ctx, `
			INSERT INTO core.ip_pools ( node_id, address, status, label)
			VALUES ( $1, $2::inet, 'free', $3)
			ON CONFLICT (node_id, address) DO NOTHING
		`, nodeID, addr, strings.TrimSpace(body.Label))
		if err == nil {
			added += int(tag.RowsAffected())
		}
	}
	audit(ctx, h.dbOf(ctx), claims.UserID, "node.ip.add", "node:"+nodeID,
...
func (h *Handler) AdminNodeIPImport(w http.ResponseWriter, r *http.Request) {
	...
	var meta []byte
	if h.dbOf(ctx).QueryRow(ctx, `SELECT COALESCE(meta, '{}'::jsonb) FROM core.nodes WHERE id = $1`,
		nodeID).Scan(&meta) != nil {
		writeError(w, http.StatusNotFound, "location not found")
		return
	}
	...
	for _, addr := range pool {
		...
		tag, err := h.dbOf(ctx).Exec(ctx, `
			INSERT INTO core.ip_pools ( node_id, address, status, label)
			VALUES ( $1, $2::inet, 'free', 'из настроек локации')
			ON CONFLICT (node_id, address) DO NOTHING
		`, nodeID, addr)
		if err == nil {
			added += int(tag.RowsAffected())
...
func (h *Handler) AdminServerAssignIP(w http.ResponseWriter, r *http.Request) {
	...
	if h.dbOf(ctx).QueryRow(ctx, `SELECT node_id::text, game_id FROM core.servers WHERE id = $1`,
		serverID).Scan(&nodeID, &gameID) != nil {
	...
	var address string
	err = tx.QueryRow(ctx, `
		UPDATE core.ip_pools SET status = 'assigned', server_id = $2::uuid, assigned_at = now()
		WHERE id = $1 AND node_id = $3 AND server_id IS NULL AND status = 'free'
	...
	audit(ctx, h.dbOf(ctx), claims.UserID, "server.ip.assign", "server:"+serverID,
...
func (h *Handler) AdminServerReleaseIP(w http.ResponseWriter, r *http.Request) {
	...
	audit(ctx, h.dbOf(ctx), claims.UserID, "server.ip.release", "server:"+serverID, nil)
	writeJSON(w, http.StatusOK, map[string]any{
		"address": fqdn, "port": port, "restart_required": true,
	})
}

func (h *Handler) serverBindIP(ctx context.Context, serverID string) string {
	var address string
