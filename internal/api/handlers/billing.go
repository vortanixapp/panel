...
func (h *Handler) TopupForm(w http.ResponseWriter, r *http.Request) {
	...
	rows, _ := h.dbOf(r.Context()).Query(r.Context(), `
		SELECT id::text, provider, enabled FROM core.payment_providers WHERE enabled = true
	`)
...
func (h *Handler) CreateTopup(w http.ResponseWriter, r *http.Request) {
	...
	if body.ProviderID != "" {
		_ = h.dbOf(r.Context()).QueryRow(r.Context(), `
			SELECT provider FROM core.payment_providers WHERE id = $1
		`, body.ProviderID).Scan(&provider)
	}
	currency := "RUB"
	var walletID *string
	if body.WalletID != "" {
	...
	var promoID string
	if body.PromoCode != "" {
		promo, err := payments.ApplyPromo(r.Context(), h.dbOf(r.Context()), claims.UserID, body.PromoCode, body.Amount)
		if err != nil {
	...
	err := h.dbOf(r.Context()).QueryRow(r.Context(), `
		...
		RETURNING id::text
	`, claims.UserID, walletID, provider, body.Amount, currency, meta, promoID).Scan(&id)
	if err != nil {
	...
	if payments.IsSupported(provider) {
		cfg, cfgErr := payments.LoadProviderConfig(r.Context(), h.dbOf(r.Context()), h.secrets, provider)
		if cfgErr == nil {
...
func (h *Handler) ListPaymentProviders(w http.ResponseWriter, r *http.Request) {
	...
	rows, _ := h.dbOf(r.Context()).Query(r.Context(), `SELECT id::text, provider, enabled FROM core.payment_providers`)
	defer func() {
		if rows != nil {
			rows.Close()
		}
...
func (h *Handler) UpdatePaymentProvider(w http.ResponseWriter, r *http.Request) {
	...
	_, _ = h.dbOf(r.Context()).Exec(r.Context(), `UPDATE core.payment_providers SET enabled = COALESCE($2, enabled), config = COALESCE($3::jsonb, config) WHERE id = $1`,
		chi.URLParam(r, "id"), body["enabled"], configJSON)
