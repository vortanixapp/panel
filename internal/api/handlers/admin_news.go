...
// Транслитерация для автослага. Заголовки у новостей русские, а Str::slug из
// старой панели умел переводить кириллицу в латиницу — без этого слаг у любой
// русской новости вышел бы пустым.
...
// newsSlugify собирает слаг из заголовка.
func newsSlugify(title string) string {
	...
	for _, r := range strings.ToLower(strings.TrimSpace(title)) {
		...
		case slugTranslit[r] != "":
			...
			prevDash = false
		case r == 'ъ' || r == 'ь':
			// Мягкий и твёрдый знаки просто выпадают, разделителя не дают.
		default:
			if !prevDash && b.Len() > 0 {
				b.WriteRune('-')
				prevDash = true
...
// newsSlugOrFallback: у новости с заголовком из одних символов слаг вышел бы
// пустым, а он обязателен и уникален.
...
type newsBody struct {
	...
	// Рубрика и закрепление. Лента фильтруется по рубрике и выносит
	// закреплённую новость наверх — без этих полей и то и другое пришлось бы
	// придумывать на клиенте.
	Tag    *string `json:"tag"`
	Pinned *bool   `json:"pinned"`
}

// validateNews повторяет правила старой панели: заголовок обязателен, длины
// ограничены. Раньше не проверялось ничего — новость с пустым заголовком
// создавалась молча.
...
// parseNewsDate отличает «поле не пришло» от «дату сняли».
...
// UpdateNews — отдельное обновление новости.
//
// Раньше его не было вовсе: POST делал upsert по slug, поэтому сменить slug
// было нельзя (создалась бы вторая запись), снять дату публикации тоже нельзя
// (COALESCE не даёт занулить), а active не трогался ничем.
func (h *Handler) UpdateNews(w http.ResponseWriter, r *http.Request) {
	...
	if body.Title != nil || body.Excerpt != nil || body.Slug != nil {
		...
		// Заголовок проверяем только если его прислали: обновление одной
		// галочки active не обязано тащить с собой весь текст.
		if body.Title != nil {
	...
	tag, err := h.dbOf(r.Context()).Exec(r.Context(), `
		UPDATE core.news SET
			...
			published_at = CASE WHEN $7::bool THEN $8::timestamptz ELSE published_at END,
			active       = COALESCE($9, active),
			tag          = COALESCE(NULLIF($10, ''), tag),
	...
	audit(r.Context(), h.dbOf(r.Context()), claims.TenantID, claims.UserID, "news.update", "news:"+id, nil)
...
// UploadNewsImage принимает изображения новости.
func (h *Handler) UploadNewsImage(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.dbOf(r.Context()).QueryRow(r.Context(),
		`SELECT slug FROM core.news WHERE id = $1::uuid AND tenant_id = $2`, id, claims.TenantID).Scan(&slug); err != nil {
		writeError(w, http.StatusNotFound, "новость не найдена")
		return
	}
	...
	err = h.dbOf(r.Context()).QueryRow(r.Context(), `
		INSERT INTO core.news_images (tenant_id, news_id, path, sort)
		VALUES ($1, $2::uuid, $3, COALESCE((SELECT MAX(sort) + 1 FROM core.news_images WHERE news_id = $2::uuid), 0))
		RETURNING id::text
	`, claims.TenantID, id, rel).Scan(&imageID)
	if err != nil {
...
// DeleteNewsImage убирает изображение вместе с файлом.
func (h *Handler) DeleteNewsImage(w http.ResponseWriter, r *http.Request) {
	...
	// Принадлежность картинки именно этой новости проверяем явно: иначе по
	// чужому id можно было бы удалить картинку другой записи.
	var rel string
	if err := h.dbOf(r.Context()).QueryRow(r.Context(), `
		DELETE FROM core.news_images
		WHERE id = $1::uuid AND news_id = $2::uuid AND tenant_id = $3
		RETURNING path
	`, imageID, newsID, claims.TenantID).Scan(&rel); err != nil {
...
// ServeNewsImage отдаёт изображение без авторизации: новости видны и на
...
// newsImages возвращает адреса картинок новостей одним запросом на список.
func (h *Handler) newsImages(r *http.Request, tenantID string, newsIDs []string) map[string][]map[string]string {
	...
	rows, err := h.dbOf(r.Context()).Query(r.Context(), `
		...
		WHERE tenant_id = $1 AND news_id = ANY($2::uuid[])
