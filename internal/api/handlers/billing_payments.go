...
func (h *Handler) completeTopupPayment(ctx context.Context, paymentID, providerPaymentID, providerLabel string) error {
	...
	if promoID != nil {
		// Условие по max_uses обязательно: без него счётчик перебивал лимит
		// гонкой одновременных вебхуков. Зачисление при этом не отменяем —
		// деньги уже получены, и отказать клиенту из-за исчерпанного лимита
		// значило бы забрать платёж ни за что.
		_, _ = tx.Exec(ctx, `
	...
	// Клиент платил и до сих пор не получал никакого подтверждения: событие
	// уходило только в вебхук, который настраивает владелец панели, а не клиент.
	h.notifyUser(ctx, tenantID, userID, notify.Event{
		...
		// Провайдеры повторяют уведомления о платеже; сам зачёт защищён условием
		// в UPDATE, а ключ защищает от повторного письма.
