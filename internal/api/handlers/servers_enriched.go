...
func (h *Handler) getEnrichedServer(ctx context.Context, userID, role, serverID string) (map[string]any, *serverAccess, bool) {
	...
	q := `
		...
		LEFT JOIN core.users u ON u.id = s.user_id
		WHERE s.id = $1::uuid`
	row := h.dbOf(ctx).QueryRow(ctx, q, serverID)
	var id, name, ip, status, runtime, prov, provError, gameID string
	var gameName, gameSlug, gameImage, locName, country, city, tariffName *string
	var ownerEmail, blockedReason string
	var isBlocked bool
...
func (h *Handler) GetServerDetail(w http.ResponseWriter, r *http.Request) {
	...
	item, access, found := h.getEnrichedServer(r.Context(), claims.UserID, claims.Role, id)
	if !found {
...
func (h *Handler) enrichServerDetail(ctx context.Context, serverID string, item map[string]any, access *serverAccess) {
...
func (h *Handler) listActiveGameVersions(ctx context.Context, gameSlug string) []map[string]any {
	if gameSlug == "" {
		return []map[string]any{}
	}
	rows, err := h.dbOf(ctx).Query(ctx, `
...
func (h *Handler) gameSteamUpdatable(ctx context.Context, gameSlug string) bool {
	if gameSlug == "" {
		return false
	}
	var meta []byte
...
func (h *Handler) MyServers(w http.ResponseWriter, r *http.Request) {
	...
	_ = h.dbOf(ctx).QueryRow(ctx, `SELECT COUNT(*) FROM core.servers WHERE true`+ownerFilter, args...).Scan(&total)
	_ = h.dbOf(ctx).QueryRow(ctx, `
		SELECT COUNT(*) FROM core.servers
		WHERE true`+ownerFilter+` AND (status = 'running' OR runtime_status = 'running')
	`, args...).Scan(&active)
