...
func (h *Handler) AdminAgentUpdates(w http.ResponseWriter, r *http.Request) {
	if !updatesStaff(w, r) {
		return
	}
	ctx := r.Context()
	...
	rows, err := h.readerOf(ctx).Query(ctx, `
		SELECT n.id::text, n.name, COALESCE(n.meta->>'code', n.fqdn, ''), COALESCE(n.country, ''),
			n.agent_auto_update, n.agent_update,
			COALESCE(d.status, 'unknown'), COALESCE(d.version, ''), d.last_seen_at,
	...
	for rows.Next() {
		var id, name, code, country, daemonStatus, version string
		var auto, ssh bool
		var updateRaw []byte
		var lastSeen *time.Time
		if err := rows.Scan(&id, &name, &code, &country, &auto, &updateRaw, &daemonStatus, &version, &lastSeen, &ssh); err != nil {
			continue
		}
		version = buildinfo.Normalize(version)
		item := map[string]any{
			"id": id, "name": name, "code": code, "country": country,
			"online":      agentDaemonOnline(daemonStatus, lastSeen),
			"version":     version,
			"outdated":    version != "" && updates.AgentOutdated(version, target),
			"auto_update": auto,
			"ssh":         ssh,
		}
		if lastSeen != nil {
		...
		if st := agentUpdateView(updateRaw); st != nil {
			item["update"] = st
		}
...
func (h *Handler) AdminStartAgentUpdates(w http.ResponseWriter, r *http.Request) {
	claims, ok := tenantClaims(r.Context())
	if !ok || !isStaffRole(claims.Role) {
		writeError(w, http.StatusForbidden, "forbidden")
		return
	...
	for _, id := range body.NodeIDs {
		method, err := h.startAgentUpdate(ctx, strings.TrimSpace(id), target, image, "manual")
	...
	audit(ctx, h.dbOf(ctx), claims.UserID, "updates.agents.start", "nodes", map[string]any{
		"nodes": body.NodeIDs, "target": target, "started": started,
	})
	writeJSON(w, http.StatusOK, map[string]any{"results": results, "started": started})
}

func (h *Handler) AdminSetAgentAutoUpdate(w http.ResponseWriter, r *http.Request) {
	claims, ok := tenantClaims(r.Context())
	if !ok || !isStaffRole(claims.Role) {
		writeError(w, http.StatusForbidden, "forbidden")
		return
	...
	if len(body.NodeIDs) == 0 {
		_, err = h.dbOf(ctx).Exec(ctx, `UPDATE core.nodes SET agent_auto_update = $1`, *body.AutoUpdate)
	} else {
		_, err = h.dbOf(ctx).Exec(ctx, `
			UPDATE core.nodes SET agent_auto_update = $1 WHERE id::text = ANY($2::text[])
		`, *body.AutoUpdate, body.NodeIDs)
	}
	if err != nil {
		writeError(w, http.StatusInternalServerError, "database error")
		return
	}
	audit(ctx, h.dbOf(ctx), claims.UserID, "updates.agents.auto", "nodes", map[string]any{
		"nodes": body.NodeIDs, "auto_update": *body.AutoUpdate,
	})
