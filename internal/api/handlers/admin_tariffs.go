...
const tariffSelectSQL = `
	SELECT t.id::text, t.node_id::text, t.game_id::text,
...
func (h *Handler) loadTariffRow(r *http.Request, id string) (*tariffRow, error) {
	q := tariffSelectSQL + ` WHERE t.id = $1`
	return scanTariffRow(h.readerOf(r.Context()).QueryRow(r.Context(), q, id))
}
...
func (h *Handler) AdminTariffCreateForm(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]any{
		"locations": h.adminTariffLocations(r),
		"games":     h.adminTariffGames(r),
	})
}

func (h *Handler) adminTariffLocations(r *http.Request) []map[string]any {
	rows, _ := h.readerOf(r.Context()).Query(r.Context(), `
		SELECT id::text, name FROM core.nodes ORDER BY sort_order, name
	`)
	defer func() {
		if rows != nil {
			rows.Close()
...
func (h *Handler) adminTariffGames(r *http.Request) []map[string]any {
	rows, _ := h.readerOf(r.Context()).Query(r.Context(), `
		SELECT id::text, name FROM core.games ORDER BY name
	`)
	defer func() {
		if rows != nil {
			rows.Close()
...
func (h *Handler) GetAdminTariff(w http.ResponseWriter, r *http.Request) {
	...
	row, err := h.loadTariffRow(r, chi.URLParam(r, "id"))
	if err != nil {
