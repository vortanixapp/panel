...
func (h *Handler) AdminSearch(w http.ResponseWriter, r *http.Request) {
	...
	if rows, err := h.readerOf(ctx).Query(ctx, `
		...
		WHERE p.tenant_id = $1
		  AND (COALESCE(p.provider_payment_id, '') ILIKE '%' || $2 || '%'
	...
	if rows, err := h.readerOf(ctx).Query(ctx, `
		...
		WHERE t.tenant_id = $1
		  AND (t.subject ILIKE '%' || $2 || '%' OR COALESCE(u.email, '') ILIKE '%' || $2 || '%')
	...
	if rows, err := h.readerOf(ctx).Query(ctx, `
		...
		FROM core.nodes
		WHERE tenant_id = $1
		  AND (name ILIKE '%' || $2 || '%' OR COALESCE(fqdn, '') ILIKE '%' || $2 || '%')
		ORDER BY name
		LIMIT 5
	`, claims.TenantID, query); err == nil {
		for rows.Next() {
