...
func (h *Handler) adminHostingPanelRef(r *http.Request, tenantID, accountID string) (string, hosting.ServerConfig, error) {
	...
	`, accountID, tenantID).Scan(&panelID, &cfg.PanelType, &cfg.APIURL, &cfg.APIUsername, &cfg.APIToken)
	if err != nil {
		return "", cfg, err
	}
...
func (h *Handler) AdminHostingAccountSuspend(w http.ResponseWriter, r *http.Request) {
	...
	panelID, cfg, err := h.adminHostingPanelRef(r, claims.TenantID, accountID)
	if err != nil {
	...
	audit(r.Context(), h.dbOf(r.Context()), claims.TenantID, claims.UserID, "hosting.suspend",
...
func (h *Handler) AdminHostingAccountUnsuspend(w http.ResponseWriter, r *http.Request) {
	...
	_, _ = h.dbOf(r.Context()).Exec(r.Context(), `
		...
		WHERE id = $1 AND tenant_id = $2
	`, accountID, claims.TenantID)
	audit(r.Context(), h.dbOf(r.Context()), claims.TenantID, claims.UserID, "hosting.unsuspend",
