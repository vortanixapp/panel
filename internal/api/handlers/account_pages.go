...
func (h *Handler) ListActivity(w http.ResponseWriter, r *http.Request) {
	...
	where, args := h.activityWhere(claims.TenantID, claims.UserID, claims.Role, f)
	...
	writeJSON(w, http.StatusOK, map[string]any{
		...
		"stats":    h.activityStats(ctx, claims.TenantID, claims.UserID, claims.Role),
		"range":    f.Days,
	})
}

func (h *Handler) activityWhere(
	tenantID, userID, role string, f activityFilters,
) (string, []any) {
	args := []any{tenantID}
	where := "a.tenant_id = $1"
...
func (h *Handler) activityStats(ctx context.Context, tenantID, userID, role string) map[string]any {
	scope := "a.tenant_id = $1"
	args := []any{tenantID}
	if !isStaffRole(role) {
		args = append(args, userID)
...
type bonusPrize struct {
	...
	// Вид скидки промо-приза: процент или фиксированная сумма. Без него приз
	// «−20%» и приз «−20 ₽» неразличимы.
	DiscountType string `json:"discount_type"`
}

// promoScopesForPrize — на что действует промокод, выданный колесом.
//
// Значения совпадают с payments.Apply*: по ним PickPromotion решает, можно ли
// применить код в аренде, продлении или хостинге.
func promoScopesForPrize(prizeType string) []string {
	...
	case "promo_game":
		// Приз на игровой сервер: и первая аренда, и продление уже взятого.
...
// bonusPromoCode — код вида BONUS-XXXXXXXX. Буквы без похожих на цифры (I, O),
// чтобы код можно было продиктовать и переписать с экрана.
...
func (h *Handler) loadBonusPrizes(ctx context.Context, tenantID string) []bonusPrize {
	rows, err := h.readerOf(ctx).Query(ctx, `
...
func (h *Handler) lastSpinAt(ctx context.Context, tenantID, userID string) *time.Time {
	var last *time.Time
	_ = h.readerOf(ctx).QueryRow(ctx, `
...
func (h *Handler) bonusStreak(ctx context.Context, tenantID, userID string) (int, map[string]bool) {
...
func (h *Handler) bonusHistory(ctx context.Context, tenantID, userID string, limit int) []bonusHistoryItem {
...
func (h *Handler) DailyBonusIndex(w http.ResponseWriter, r *http.Request) {
	...
	prizes := h.loadBonusPrizes(ctx, claims.TenantID)
	last := h.lastSpinAt(ctx, claims.TenantID, claims.UserID)
	streak, days := h.bonusStreak(ctx, claims.TenantID, claims.UserID)
	...
	writeJSON(w, http.StatusOK, map[string]any{
		...
		"streak":          streak,
		"streak_days":     week,
		"history":         h.bonusHistory(ctx, claims.TenantID, claims.UserID, 10),
...
func (h *Handler) DailyBonusSpin(w http.ResponseWriter, r *http.Request) {
	...
	if last := h.lastSpinAt(ctx, claims.TenantID, claims.UserID); last != nil {
	...
	credited, promoCode, err := h.grantBonusPrize(ctx, claims.TenantID, claims.UserID, prize)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "failed to grant prize")
		return
	}

	auditWithIP(ctx, r, h.dbOf(ctx), claims.TenantID, claims.UserID, "bonus.spin", "daily_bonus",
	...
	streak, _ := h.bonusStreak(ctx, claims.TenantID, claims.UserID)
	writeJSON(w, http.StatusOK, map[string]any{
		"ok":           true,
		"prize_id":     prize.ID,
		"prize":        prize,
...
func (h *Handler) grantBonusPrize(
	ctx context.Context, tenantID, userID string, prize bonusPrize,
	...
	if prize.Type == "balance" && prize.Value > 0 {
		...
		`, tenantID, userID).Scan(&walletID)

		if err != nil {
			if err := tx.QueryRow(ctx, `
				INSERT INTO core.wallets (tenant_id, user_id, currency, balance, is_default)
				VALUES ($1, $2, 'RUB', 0, true)
				RETURNING id::text
			`, tenantID, userID).Scan(&walletID); err != nil {
				return 0, "", err
			}
		}
		...
		if _, err := tx.Exec(ctx, `
			INSERT INTO core.transactions
			    (tenant_id, wallet_id, type, amount, description, source_type)
			VALUES ($1, $2, 'credit', $3, $4, 'daily_bonus')
		`, tenantID, walletID, prize.Value, "Ежедневный бонус: "+prize.Label); err != nil {
			return 0, "", err
		}
		credited = prize.Value
	}

	// Промо-призы раньше не выдавали ничего: колесо списывало попытку и писало
	// красивую строку в историю, а скидки не появлялось. Теперь под приз
	// заводится личный промокод — одноразовый и только для этого пользователя.
	promoID, promoCode := "", ""
	if scopes := promoScopesForPrize(prize.Type); len(scopes) > 0 && prize.Value > 0 {
		...
		// Код случайный, но UNIQUE (tenant_id, code) в базе. Несколько попыток
		// вместо одной: столкновение маловероятно, а терять приз из-за него нельзя.
		for attempt := 0; attempt < 5 && promoID == ""; attempt++ {
			code := bonusPromoCode()
			err := tx.QueryRow(ctx, `
				INSERT INTO core.promotions (
					tenant_id, title, code, active, starts_at, ends_at, applies_to,
					discount_type, discount_value, max_uses, filters, description
				) VALUES ($1, $2, $3, true, now(), $4, $5::jsonb, $6, $7, 1, $8::jsonb, $9)
				ON CONFLICT (tenant_id, code) DO NOTHING
				RETURNING id::text
			`, tenantID, "Ежедневный бонус: "+prize.Label, code, endsAt, scopesJSON,
	...
	if _, err := tx.Exec(ctx, `
		INSERT INTO core.daily_bonus_spins
		    (tenant_id, user_id, prize_id, prize_snapshot, reward_type, reward_value,
		     promotion_id, promotion_code)
		VALUES ($1, $2, $3::uuid, $4::jsonb, $5, $6, NULLIF($7, '')::uuid, NULLIF($8, ''))
	`, tenantID, userID, prize.ID, snapshot, prize.Type, prize.Value,
...
func (h *Handler) loadNotificationChannels(ctx context.Context, tenantID, userID string) notificationChannels {
	...
	`, userID, tenantID).Scan(&c.Email, &c.Telegram, &c.Discord, &c.TelegramChatID, &c.DiscordWebhook)
	return c
}
...
func (h *Handler) NotificationChannelsShow(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]any{
		"channels": h.loadNotificationChannels(r.Context(), claims.TenantID, claims.UserID),
	})
}
...
func (h *Handler) NotificationChannelsUpdate(w http.ResponseWriter, r *http.Request) {
	...
	ctx := r.Context()
	current := h.loadNotificationChannels(ctx, claims.TenantID, claims.UserID)
	...
	if _, err := h.dbOf(ctx).Exec(ctx, `
		INSERT INTO core.user_notification_channels
		    (user_id, tenant_id, email_enabled, telegram_enabled, discord_enabled,
		...
		ON CONFLICT (user_id) DO UPDATE SET
		    ...
		    updated_at       = now()
	`, claims.UserID, claims.TenantID, current.Email, current.Telegram, current.Discord,
...
func auditWithIP(
	...
	tenantID, userID, action, resource string, meta map[string]any,
) {
	if meta == nil {
		meta = map[string]any{}
	}
