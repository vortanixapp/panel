...
func (h *Handler) AdminJobsList(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.readerOf(ctx).Query(ctx, `
		...
		LEFT JOIN core.servers s
		       ON s.tenant_id = j.tenant_id AND s.id = core.try_uuid(j.payload->>'server_id')
		LEFT JOIN core.nodes n
		       ON n.tenant_id = j.tenant_id
		      AND n.id = COALESCE(core.try_uuid(j.payload->>'node_id'), s.node_id)
	`+where+`
	...
	writeJSON(w, http.StatusOK, map[string]any{
		...
		"summary":       h.jobsSummary(ctx, claims.TenantID, stuckMinutes),
		"types":         h.jobsTypes(ctx, claims.TenantID),
	})
}

func (h *Handler) jobsSummary(ctx context.Context, tenantID string, stuckMinutes int) map[string]any {
	summary := map[string]any{
		"pending": 0, "running": 0, "completed": 0, "failed": 0, "cancelled": 0,
		"stuck": 0, "oldest_pending_seconds": 0,
	}
	rows, err := h.readerOf(ctx).Query(ctx, `
		SELECT status, COUNT(*) FROM core.jobs WHERE tenant_id = $1 GROUP BY status
	`, tenantID)
	if err == nil {
		defer rows.Close()
		for rows.Next() {
	...
	`, tenantID, stuckMinutes).Scan(&stuck)
	summary["stuck"] = stuck

	var oldest int64
	_ = h.readerOf(ctx).QueryRow(ctx, `
		SELECT COALESCE(EXTRACT(EPOCH FROM (now() - MIN(created_at)))::bigint, 0)
		FROM core.jobs WHERE tenant_id = $1 AND status = 'pending'
	`, tenantID).Scan(&oldest)
	summary["oldest_pending_seconds"] = oldest
	return summary
}

func (h *Handler) jobsTypes(ctx context.Context, tenantID string) []map[string]any {
	out := []map[string]any{}
	rows, err := h.readerOf(ctx).Query(ctx, `
		SELECT type,
		       COUNT(*),
		...
		FROM core.jobs WHERE tenant_id = $1
...
func (h *Handler) AdminJobRetry(w http.ResponseWriter, r *http.Request) {
	...
	err := h.dbOf(r.Context()).QueryRow(r.Context(), `
		...
		RETURNING type
	`, id, claims.TenantID, nullableUUID(claims.UserID), stuckMinutes).Scan(&jobType)
	if err != nil {
		var status string
		if h.dbOf(r.Context()).QueryRow(r.Context(), `SELECT status FROM core.jobs WHERE id = $1 AND tenant_id = $2`,
			id, claims.TenantID).Scan(&status) == nil {
	...
	audit(r.Context(), h.dbOf(r.Context()), claims.TenantID, claims.UserID, "job.retry", "job:"+id,
...
func (h *Handler) AdminJobCancel(w http.ResponseWriter, r *http.Request) {
	...
	err := h.dbOf(r.Context()).QueryRow(r.Context(), `
		...
		WHERE id = $1 AND tenant_id = $2
		  AND (status = 'pending'
		       OR (status = 'running' AND updated_at < now() - make_interval(mins => $5::int)))
		RETURNING type
	`, id, claims.TenantID, result, nullableUUID(claims.UserID), stuckMinutes).Scan(&jobType)
	if err != nil {
		var status string
		if h.dbOf(r.Context()).QueryRow(r.Context(), `SELECT status FROM core.jobs WHERE id = $1 AND tenant_id = $2`,
			id, claims.TenantID).Scan(&status) == nil {
	...
	audit(r.Context(), h.dbOf(r.Context()), claims.TenantID, claims.UserID, "job.cancel", "job:"+id,
		map[string]any{"type": jobType})
...
func (h *Handler) AdminJobDelete(w http.ResponseWriter, r *http.Request) {
	...
	audit(r.Context(), h.dbOf(r.Context()), claims.TenantID, claims.UserID, "job.delete", "job:"+id, nil)
...
func (h *Handler) AdminJobsRetryFailed(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.dbOf(r.Context()).Query(r.Context(), `
		...
		WHERE tenant_id = $1 AND status = 'failed'
		  AND ($3 = '' OR type = $3)
		RETURNING type
	`, claims.TenantID, nullableUUID(claims.UserID), jobType)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "database error")
		return
	}
	...
	audit(r.Context(), h.dbOf(r.Context()), claims.TenantID, claims.UserID, "job.retry_failed", "jobs",
...
func (h *Handler) AdminJobsCleanup(w http.ResponseWriter, r *http.Request) {
	...
	tag, err := h.dbOf(r.Context()).Exec(r.Context(), `
		DELETE FROM core.jobs
		WHERE tenant_id = $1 AND status IN ('completed', 'cancelled')
	...
	audit(r.Context(), h.dbOf(r.Context()), claims.TenantID, claims.UserID, "job.cleanup", "jobs",
