...
func (h *Handler) GetAdminAppearance(w http.ResponseWriter, r *http.Request) {
	...
	for _, key := range appearanceSettingKeys {
		values[key] = h.tenantSettingString(ctx, key)
	}
...
func (h *Handler) UpdateAdminAppearance(w http.ResponseWriter, r *http.Request) {
	...
	h.setTenantSettingString(ctx, "app.site.default_template", strings.TrimSpace(body.DefaultTemplate))
	h.setTenantSettingString(ctx, "app.site.template.colors", body.TemplateColors)
	h.setTenantSettingString(ctx, "app.site.template.blocks", body.TemplateBlocks)
	h.setTenantSettingString(ctx, "app.site.template.user_menu_variant", variant)
