...
func (h *Handler) SupportCreateForm(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.dbOf(r.Context()).QueryRow(r.Context(), `
		SELECT value FROM core.tenant_settings WHERE key = 'support.departments'
	...
	writeJSON(w, http.StatusOK, map[string]any{
		"departments": depts,
		"services":    h.userServices(r.Context(), claims.UserID),
		"sla":         h.supportSLA(r.Context()),
	})
}
...
func (h *Handler) UploadSupportAttachment(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := tx.Exec(ctx, `
		INSERT INTO core.support_messages (id, ticket_id, user_id, is_staff, message, body)
		SELECT $1::uuid, $2::uuid, $3::uuid, $4, $5, $5
		FROM core.support_tickets WHERE id = $2::uuid
	`, messageID, ticketID, claims.UserID, isStaff, label); err != nil {
		cleanup()
		writeError(w, http.StatusInternalServerError, "failed")
		return
	}
	...
	for _, p := range saved {
		...
		if _, err := tx.Exec(ctx, `
			INSERT INTO core.support_message_attachments (
				id, ticket_id, message_id, user_id, is_staff,
				disk, path, original_name, mime_type, size_bytes
			) VALUES ($1::uuid, $2::uuid, $3::uuid, $4::uuid, $5, 'local', $6, $7, $8, $9)
		`, p.id, ticketID, messageID, claims.UserID, isStaff,
...
func (h *Handler) DownloadSupportAttachment(w http.ResponseWriter, r *http.Request) {
	...
	_, ok := h.supportTicketAccess(r, ticketID)
	if !ok {
		writeError(w, http.StatusNotFound, "not found")
		return
	}
	...
	`, messageID, ticketID).Scan(&relPath, &name, &mime) == nil {
...
func (h *Handler) supportAttachmentsFor(ctx context.Context, ticketID, messageID string) []map[string]any {
