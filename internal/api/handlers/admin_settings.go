...
func (h *Handler) UpdateAdminSettings(w http.ResponseWriter, r *http.Request) {
	...
	h.applyBoolSetting(ctx, form, "require_verified_email", "auth.require_verified_email")
	h.applyBoolSetting(ctx, form, "server_status_notifications", "vtx_mail.server_status_notifications")
	h.applyBoolSetting(ctx, form, "telegram_notifications_enabled", "telegram.notifications.enabled")
	h.applyBoolSetting(ctx, form, "files_storage_ftp_passive", "files.storage.ftp.passive")
	h.applyBoolSetting(ctx, form, "files_storage_ftp_ssl", "files.storage.ftp.ssl")
	h.applyBoolSetting(ctx, form, "files_storage_s3_use_path_style_endpoint", "files.storage.s3.use_path_style_endpoint")

	if vals, ok := form["files_storage_ftp_password"]; ok {
		h.setTenantSettingString(ctx, "files.storage.ftp.password", h.resolveMaskedFTPPassword(ctx, vals[0]))
	}
...
func (h *Handler) AdminSettingsTestTelegram(w http.ResponseWriter, r *http.Request) {
	...
	ctx := r.Context()
	token := h.tenantSettingString(ctx, "telegram.notifications.bot_token")
	if token == "" {
	...
	chatID := h.tenantSettingString(ctx, "telegram.notifications.admin_chat_id")
	if chatID == "" {
		writeJSON(w, http.StatusUnprocessableEntity, map[string]any{"ok": false, "message": "Admin Chat ID не указан."})
		return
	}
	appName := h.tenantSettingString(ctx, "app.name")
	if appName == "" {
		appName = "Vortanix"
	}
...
func (h *Handler) AdminSettingsTestFilesStorage(w http.ResponseWriter, r *http.Request) {
	...
	default:
		...
		if pwd == maskedFTPPassword {
			pwd = h.tenantSettingString(r.Context(), "files.storage.ftp.password")
		}
...
func (h *Handler) loadTenantSettingStrings(ctx context.Context) map[string]string {
	out := map[string]string{}
	rows, err := h.dbOf(ctx).Query(ctx, `SELECT key, value FROM core.tenant_settings`)
	if err != nil {
		return out
	}
	defer rows.Close()
...
func (h *Handler) tenantSettingString(ctx context.Context, key string) string {
	var raw []byte
	if h.dbOf(ctx).QueryRow(ctx, `SELECT value FROM core.tenant_settings WHERE key = $1`, key).Scan(&raw) != nil {
		return ""
	}
	return h.secrets.MustDecrypt(jsonValueToString(raw))
}

func (h *Handler) setTenantSettingString(ctx context.Context, key, value string) {
	if secretSettingKeys[key] {
		if value == "" && h.storedSecretUnreadable(ctx, key) {
			return
		}
	...
	`, key, b); err != nil {
		log.Printf("настройка %s не сохранена: %v", key, err)
	}
}

func (h *Handler) storedSecretUnreadable(ctx context.Context, key string) bool {
	var raw []byte
	if h.dbOf(ctx).QueryRow(ctx, `SELECT value FROM core.tenant_settings WHERE key = $1`, key).Scan(&raw) != nil {
		return false
	}
...
func (h *Handler) setTenantSettingAny(ctx context.Context, key string, value any) {
	if secretSettingKeys[key] {
		if str, ok := value.(string); ok {
			h.setTenantSettingString(ctx, key, str)
			return
		}
	}
	b, _ := json.Marshal(value)
...
func (h *Handler) applyBoolSetting(ctx context.Context, form map[string][]string, formKey, settingKey string) {
	val := "0"
	if vals, ok := form[formKey]; ok && formTruthy(vals[0]) {
		val = "1"
	}
	h.setTenantSettingString(ctx, settingKey, val)
}
...
func (h *Handler) resolveMaskedFTPPassword(ctx context.Context, input string) string {
