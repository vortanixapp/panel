...
// Проверки перед установкой плагина или карты.
//
// В старой панели каждая такая операция начиналась с четырёх обязательных
// проверок, и ни одна из них не была формальностью: без них клиент ставит
// плагин на сервер с истёкшей арендой, во время переустановки (когда каталог
// вот-вот перезапишется) или плагин от другой игры, который просто не заработает.

// serverOperableState — состояние сервера, важное для операций с каталогом.
...
func (h *Handler) loadServerOperableState(ctx context.Context, tenantID, serverID string) (serverOperableState, error) {
	...
	err := h.dbOf(ctx).QueryRow(ctx, `
		...
		FROM core.servers s
		WHERE s.id = $1::uuid AND s.tenant_id = $2
	`, serverID, tenantID).Scan(&st.Expired, &st.Provisioning, &st.GameSlug)
	return st, err
}

// ensureServerAcceptsCatalogChanges повторяет abortIfExpired и
// abortIfReinstalling из старой панели.
...
// pluginInstallSpec — то, что нужно знать о плагине перед установкой.
...
func (h *Handler) loadPluginInstallSpec(ctx context.Context, tenantID, pluginID string) (pluginInstallSpec, error) {
	...
	`, pluginID, tenantID).Scan(&spec.Active, &spec.InstallPath, &spec.SupportedGames,
...
// supportsGame: пустой список означает «все игры». Старая панель записывала для
// этого случая ['*'], поэтому оба варианта считаем одним и тем же.
...
// ensurePluginInstallable — проверки перед установкой плагина на сервер.
func (spec pluginInstallSpec) ensureInstallable(gameSlug string) error {
	...
	// Плагин без архива и без действий с файлами не делает ничего: установка
	// прошла бы «успешно», не изменив на сервере ни байта.
...
// ensureUninstallable — проверки перед удалением плагина с сервера.
func (spec pluginInstallSpec) ensureUninstallable() error {
	...
	// снесло бы сервер целиком, поэтому такие плагины снимаются только своими
	// uninstall_actions.
...
// mapInstallSpec — то, что нужно знать о карте перед установкой.
...
func (h *Handler) loadMapInstallSpec(ctx context.Context, tenantID, mapID string) (mapInstallSpec, error) {
	...
	`, mapID, tenantID).Scan(&spec.Active, &archivePath, &fileList, &spec.GameSlug)
	if err != nil {
		return spec, err
	}
	spec.HasArchive = archivePath != ""
...
func (spec mapInstallSpec) ensureInstallable(serverGame string) error {
	...
	// Без списка файлов карту потом нечем будет удалить: агент не знает, что
