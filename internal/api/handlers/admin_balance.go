...
func (h *Handler) AdminAdjustUserBalance(w http.ResponseWriter, r *http.Request) {
	...
	`, claims.TenantID, userID, currency).Scan(&walletID, &balance)
	if err != nil {
	...
	if _, err := tx.Exec(r.Context(), `
		INSERT INTO core.transactions (tenant_id, wallet_id, type, amount, description, meta, source_type)
		VALUES ($1, $2, $3, $4, $5, $6::jsonb, 'admin_adjustment')
	`, claims.TenantID, walletID, txType, math.Abs(amount), comment, meta); err != nil {
		writeError(w, http.StatusInternalServerError, "database error")
		return
	}
	...
	audit(r.Context(), h.dbOf(r.Context()), claims.TenantID, claims.UserID, "billing.adjust", "user:"+userID,
		map[string]any{"amount": amount, "currency": currency, "comment": comment})
	h.notifyUser(r.Context(), claims.TenantID, userID, notify.Event{
