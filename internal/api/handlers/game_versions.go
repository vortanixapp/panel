...
func (h *Handler) resolveDockerImage(ctx context.Context, serverID string) string {
...
func (h *Handler) catalogImageForGame(ctx context.Context, gameID, image string) (string, error) {
	...
	if err := h.dbOf(ctx).QueryRow(ctx, `
		SELECT slug FROM core.games WHERE id = $1
	`, gameID).Scan(&slug); err != nil {
		return "", nil
	}
...
func (h *Handler) verifyGameTenant(ctx context.Context, gameID string) error {
	var ok bool
	err := h.dbOf(ctx).QueryRow(ctx, `
		SELECT EXISTS(SELECT 1 FROM core.games WHERE id = $1)
	`, gameID).Scan(&ok)
	if err != nil {
		return err
	}
...
func (h *Handler) ListGameVersions(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.verifyGameTenant(r.Context(), gameID); err != nil {
...
func (h *Handler) CreateGameVersion(w http.ResponseWriter, r *http.Request) {
	...
	if img, err := h.catalogImageForGame(r.Context(), gameID, dockerImage); err != nil {
...
func (h *Handler) UpdateGameVersion(w http.ResponseWriter, r *http.Request) {
	...
	if raw := bodyString(body, "docker_image"); raw != "" {
		img, err := h.catalogImageForGame(r.Context(), gameID, raw)
		if err != nil {
			writeJSON(w, http.StatusUnprocessableEntity, map[string]any{"ok": false, "error": err.Error()})
			return
		}
