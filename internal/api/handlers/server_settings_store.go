...
// Хранение настроек сервера и обмен файлами с нодой.
//
// Это то немногое, что пережило удаление прежнего слоя настроек: разбор форматов
// переехал в пакет gamesettings, описание полей — в профили игр, а здесь остались
// только доступ к базе и две обёртки над командами агента.

// serverGameSettingsRow — строка сервера в том объёме, в каком её нужно знать
// настройкам: игра, сохранённые значения, лимиты и способ тарификации.
...
// extractStoredGameSettings достаёт сохранённые значения профиля из config.
...
// mergeGameSettingsConfig сливает значения внутрь объекта конкретной игры.
//
// Именно сливает, а не подменяет: прежний вариант собирал
// jsonb_build_object(ключ, новые) и склеивал его с game_settings через ||, то
// есть весь набор настроек игры заменялся тем, что пришло в запросе. Пока панель
// отправляла разом все поля, это сходило с рук; теперь она шлёт только
// изменённые, и остальные молча исчезли бы.
//
// Чтение и запись здесь в одном операторе, поэтому два одновременных сохранения
// выстраиваются в очередь на блокировке строки, а не затирают друг друга.
...
// effectiveStartupParams достаёт строку аргументов запуска из config.
//
// Форма значения исторически бывала разной: ручка сохранения одно время писала
// туда всё тело запроса объектом. Читаем терпимо к обеим.
