...
func (h *Handler) loadServerGameSettingsRow(ctx context.Context, serverID string) (*serverGameSettingsRow, error) {
	var gameID string
	...
	err := h.dbOf(ctx).QueryRow(ctx, `
		...
		WHERE s.id = $1
	`, serverID).Scan(&gameID, &configRaw, &limitsRaw, &billingType)
	if err != nil {
		return nil, err
	}
...
func (h *Handler) mergeGameSettingsConfig(ctx context.Context, serverID, profileKey string, settings map[string]any) error {
	...
	_, err = h.dbOf(ctx).Exec(ctx, `
		...
		SET config = jsonb_set(
			...
			ARRAY['game_settings', $2::text],
			COALESCE(config->'game_settings'->$2::text, '{}'::jsonb) || $3::jsonb,
			true
		)
		WHERE id = $1
	`, serverID, profileKey, raw)
	return err
}

func (h *Handler) updateServerStartupParamsString(ctx context.Context, serverID, params string) error {
	_, err := h.dbOf(ctx).Exec(ctx, `
		UPDATE core.servers
		SET config = config || jsonb_build_object('startup_params', to_jsonb($2::text))
		WHERE id = $1
	`, serverID, params)
	return err
}
...
func (h *Handler) agentFilesReadQuiet(ctx context.Context, serverID, path string) (string, bool) {
...
func (h *Handler) agentFilesWriteQuiet(ctx context.Context, serverID, path, content string) bool {
	nodeID, err := h.serverNodeID(ctx, serverID)
	if err != nil {
		return false
	}
