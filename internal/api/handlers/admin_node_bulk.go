...
func (h *Handler) AdminNodeBulkAction(w http.ResponseWriter, r *http.Request) {
	...
	var nodeName string
	if h.dbOf(ctx).QueryRow(ctx, `SELECT name FROM core.nodes WHERE id = $1 AND tenant_id = $2`,
		nodeID, claims.TenantID).Scan(&nodeName) != nil {
		writeError(w, http.StatusNotFound, "location not found")
		return
	}

	var affected int
	countQuery := `SELECT COUNT(*)::int FROM core.servers WHERE node_id = $1 AND tenant_id = $2`
	if body.OnlyRunning {
		countQuery += ` AND status = 'running'`
	}
	_ = h.dbOf(ctx).QueryRow(ctx, countQuery, nodeID, claims.TenantID).Scan(&affected)
	if affected == 0 {
	...
	if _, err := h.dbOf(ctx).Exec(ctx, `
		...
		VALUES ($1, 'node_bulk', 'pending', $2::jsonb)
	`, claims.TenantID, payload); err != nil {
		writeError(w, http.StatusInternalServerError, "database error")
		return
	}
	jobwake.Notify("node_bulk")
	audit(ctx, h.dbOf(ctx), claims.TenantID, claims.UserID, "node.bulk."+action, "node:"+nodeID,
		map[string]any{"servers": affected, "days": body.Days})
	h.auditAlert(ctx, claims.TenantID, claims.UserID, claims.Email, "node.bulk."+action,
