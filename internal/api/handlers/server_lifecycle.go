...
func (h *Handler) UpdateServerGame(w http.ResponseWriter, r *http.Request) {
	...
	// Агенту нужен тот же набор, что и при установке: откуда брать файлы и с
	// чем поднимать контейнер обратно. Раньше уходил один флаг steam_update,
	// по которому обновить было нечего.
	...
	// Статус запоминаем до отправки: агент прежних версий эту команду не
	// разбирает и в ответ сообщает, что сервер остановлен. Работающий сервер
	// после такого числился бы остановленным, поэтому статус придётся вернуть.
	...
	// Новый агент отвечает «updating». Пустой ответ значит, что команду никто
	// не разобрал: обновление не началось, и говорить обратное нельзя.
...
func (h *Handler) ReinstallServer(w http.ResponseWriter, r *http.Request) {
	...
	serverID := chi.URLParam(r, "id")
	// Переустановка стирает данные сервера. Проверка «тот же арендатор»
	// пускала сюда любого клиента панели, знающего чужой id.
	...
	// Кэш статуса и список арендатора обновляем так же, как это делает
	// обработчик питания. Без этого переустановка первые полминуты показывала
	// прежнее состояние: эффективный статус берётся из кэша, а он оставался от
	// предыдущей операции.
	_ = h.cache.SetServerStatus(ctx, serverID, "reinstalling")
	_ = h.cache.InvalidateTenantServers(ctx, claims.TenantID)

	// Прогресс прошлой установки живёт полчаса и никем не удаляется. Если его не
	// сбросить, новая переустановка начинается с показа «готово, 100 %» и стоит
	// так до первого процента от агента — то есть ровно в тот момент, когда
	// человек ждёт обратной связи, шкала врёт.
