...
func (h *Handler) suspendExpiredHosting(ctx context.Context) {
	...
	rows, err := h.dbOf(ctx).Query(ctx, `
		...
		RETURNING ha.id::text, ha.tenant_id::text, COALESCE(ha.user_id::text, ''),
	...
	type expiredAccount struct {
		id, tenantID, userID, panelID, username, serverID string
	}
	var list []expiredAccount
	for rows.Next() {
		var a expiredAccount
		if rows.Scan(&a.id, &a.tenantID, &a.userID, &a.panelID, &a.username, &a.serverID) == nil {
			list = append(list, a)
		}
	}
	rows.Close()
	...
	for _, a := range list {
		...
		if a.userID != "" {
			h.notifyUser(ctx, a.tenantID, a.userID, notify.Event{
				...
				// Подметатель проходит по кругу и без ключа слал бы одно и то же
				// каждую итерацию, пока клиент не продлит.
		...
		_, _ = h.dbOf(ctx).Exec(ctx, `
			...
			VALUES ($1, NULL, 'hosting.suspend', $2, $3::jsonb)
		`, a.tenantID, "hosting_account:"+a.id, meta)
