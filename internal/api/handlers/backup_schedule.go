...
func (h *Handler) GetServerBackupSchedule(w http.ResponseWriter, r *http.Request) {
	...
	_, ok := h.authorizeServerTab(w, r, id, "backup_schedule_read")
	if !ok {
		return
	}
	var (
	...
	err := h.readerOf(r.Context()).QueryRow(r.Context(), `
		...
		FROM core.server_backup_schedules WHERE server_id = $1
	`, id).Scan(&enabled, &frequency, &hour, &dow, &keepCount, &lastRun, &lastErr)
	if err != nil {
...
func (h *Handler) PutServerBackupSchedule(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := h.dbOf(r.Context()).Exec(r.Context(), `
		INSERT INTO core.server_backup_schedules
			(server_id, enabled, frequency, hour_utc, day_of_week, keep_count)
		VALUES ($1, $2, $3, $4, $5, $6)
		ON CONFLICT (server_id) DO UPDATE SET
			...
			updated_at  = now()
	`, id, enabled, frequency, hour, dow, keep); err != nil {
		writeError(w, http.StatusInternalServerError, "database error")
		return
	}
	audit(r.Context(), h.dbOf(r.Context()), claims.UserID, "server.backup_schedule", "server:"+id,
