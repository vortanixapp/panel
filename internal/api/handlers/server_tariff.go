...
func (h *Handler) ServerTariffList(w http.ResponseWriter, r *http.Request) {
	...
	`, serverID, claims.TenantID).Scan(&nodeID, &gameID, &tariffID)
	if err != nil {
		writeError(w, http.StatusNotFound, "server not found")
		return
	}
	...
	q := tariffSelectSQL + `
		WHERE t.tenant_id = $1 AND t.active = true
		  AND (t.node_id IS NULL OR t.node_id::text = $2)
		  AND (t.game_id IS NULL OR t.game_id::text IN (
		    SELECT g.id::text FROM core.games g WHERE g.tenant_id = $1 AND g.slug = $3
		  ))
		ORDER BY t.position ASC, t.name ASC`
	rows, err := h.dbOf(r.Context()).Query(r.Context(), q, claims.TenantID, nodeID, gameID)
	if err != nil {
...
func (h *Handler) ServerTariffChange(w http.ResponseWriter, r *http.Request) {
	...
	ctx := r.Context()
	row, err := h.loadServerTariffRow(ctx, claims.TenantID, serverID)
	if err != nil {
		writeError(w, http.StatusNotFound, "server not found")
		return
	}
	...
	currentTariff, err := h.loadTariffLegacyJSON(ctx, claims.TenantID, row.tariffID)
	if err != nil {
		writeError(w, http.StatusBadRequest, "Тариф не найден")
		return
	}
	newTariff, err := h.loadTariffLegacyJSON(ctx, claims.TenantID, newTariffID)
	if err != nil {
		writeError(w, http.StatusBadRequest, "Тариф не найден")
		return
	}
	...
	_, err = h.dbOf(ctx).Exec(ctx, `
		UPDATE core.servers
		SET tariff_id = $3::uuid, limits = $4::jsonb, expires_at = $5
		WHERE id = $1 AND tenant_id = $2
	`, serverID, claims.TenantID, newTariffID, limitsJSON, newExpires)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "update failed")
		return
	}
	audit(ctx, h.dbOf(ctx), claims.TenantID, claims.UserID, "server.tariff_change", serverID, map[string]any{
...
func (h *Handler) ServerTariffResources(w http.ResponseWriter, r *http.Request) {
	...
	tariffJSON, err := h.loadTariffLegacyJSON(ctx, claims.TenantID, row.tariffID)
	if err != nil {
		writeError(w, http.StatusBadRequest, "Тариф не найден")
		return
	}
	...
	limitsJSON, _ := json.Marshal(limits)
	_, err = h.dbOf(ctx).Exec(ctx, `
		UPDATE core.servers SET limits = $3::jsonb, expires_at = $4 WHERE id = $1 AND tenant_id = $2
	`, serverID, claims.TenantID, limitsJSON, newExpires)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "update failed")
		return
	}
	audit(ctx, h.dbOf(ctx), claims.TenantID, claims.UserID, "server.tariff_resources", serverID, map[string]any{"delta": delta})
	writeJSON(w, http.StatusOK, map[string]any{
		"status":   "updated",
		"charged":  delta,
		"currency": currency,
...
func (h *Handler) loadServerTariffRow(ctx context.Context, tenantID, serverID string) (*serverTariffRow, error) {
	var limitsRaw []byte
	...
	err := h.dbOf(ctx).QueryRow(ctx, `
		...
		FROM core.servers WHERE id = $1 AND tenant_id = $2
	`, serverID, tenantID).Scan(&tariffID, &nodeID, &gameID, &limitsRaw, &row.expiresAt)
	if err != nil {
		return nil, err
	}
	if tariffID != nil {
