...
func (h *Handler) remindStage(ctx context.Context, stage int) {
	rows, err := h.dbOf(ctx).Query(ctx, `
		...
		RETURNING s.id::text, s.user_id::text, s.name, s.expires_at,
	...
	for rows.Next() {
		var d dunningServer
		if rows.Scan(&d.id, &d.userID, &d.name, &d.expiresAt, &d.period) == nil {
			list = append(list, d)
		}
	}
	rows.Close()
...
func (h *Handler) notifySuspended(ctx context.Context) {
	...
	for _, d := range list {
		...
		h.emitWebhook(ctx, "server.suspended", map[string]any{
...
func (h *Handler) renewAutoServers(ctx context.Context) {
	rows, err := h.dbOf(ctx).Query(ctx, `
		SELECT id::text, COALESCE(user_id::text, ''), name, expires_at,
	...
	for rows.Next() {
		var d dunningServer
		if rows.Scan(&d.id, &d.userID, &d.name, &d.expiresAt, &d.period) == nil && d.userID != "" {
			list = append(list, d)
		}
	}
	rows.Close()
...
func (h *Handler) ServerAutoRenew(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := h.dbOf(r.Context()).Exec(r.Context(), `
		UPDATE core.servers SET auto_renew = $2 WHERE id = $1
	`, serverID, body.Enabled); err != nil {
		writeError(w, http.StatusInternalServerError, "database error")
		return
	}
	audit(r.Context(), h.dbOf(r.Context()), claims.UserID, "server.auto_renew", "server:"+serverID,
