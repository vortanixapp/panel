...
// onlinePointsByBucket сворачивает опросы игрового сервера в те же вёдра, что
// и метрики. Онлайн пишет сэмплер в core.server_online_points, поэтому в
// точках метрик его нет и не было.
...
func (h *Handler) buildMonitoringStatsSeries(ctx context.Context, serverID string, days int) map[string]any {
	...
	if err != nil {
		// Молчаливый откат на Redis прятал отказ базы: история за неделю
		// строилась из получаса живых точек и выглядела как «данных нет».
	...
	// Онлайн живёт в своей таблице: его пишет опрос игровых серверов, а не
	// сборщик метрик. Раньше история искала его среди полей метрики, не
	// находила и показывала ноль на каждой строке.
	...
	// Ведро с онлайном, но без метрик, тоже строка истории: сервер могли
	// опрашивать, когда сборщик метрик молчал.
	for k := range online {
	...
	for _, k := range keys {
		...
		if o, ok := online[k]; ok {
			...
			// Различаем «сервер опрашивали, игроков не было» и «опроса не
			// было»: раньше во второй строке тоже стоял ноль.
