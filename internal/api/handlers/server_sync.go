...
func (h *Handler) loadExtraPorts(ctx context.Context, serverID string) ([]map[string]any, error) {
	rows, err := h.dbOf(ctx).Query(ctx, `
...
func (h *Handler) pushPortsState(ctx context.Context, nodeID, serverID string) error {
	ports, err := h.loadExtraPorts(ctx, serverID)
	if err != nil {
		return err
	}
...
func (h *Handler) restoreServerStatus(ctx context.Context, serverID, status, runtime string) {
	if status == "" && runtime == "" {
		return
	}
	_, _ = h.dbOf(ctx).Exec(ctx, `
		UPDATE core.servers
		SET status = NULLIF($2, ''), runtime_status = NULLIF($3, '')
		WHERE id = $1
	`, serverID, status, runtime)
}
...
func (h *Handler) syncPortsForServerHTTP(w http.ResponseWriter, r *http.Request, serverID string) bool {
	...
	if err := h.pushPortsState(r.Context(), nodeID, serverID); err != nil {
