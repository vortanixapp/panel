...
func (h *Handler) settingsFileTarget(
	ctx context.Context, serverID, fileID string,
...
func (h *Handler) ServerSettingsFilePut(w http.ResponseWriter, r *http.Request) {
	...
	if strings.TrimSpace(previous) != "" {
		...
		if !h.agentFilesWriteQuiet(ctx, serverID, backup, previous) {
	...
	if !h.agentFilesWriteQuiet(ctx, serverID, resolved, body.Content) {
		writeJSON(w, http.StatusBadGateway, map[string]any{
