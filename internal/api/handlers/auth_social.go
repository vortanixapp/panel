...
func (h *Handler) loginOrRegisterSocial(w http.ResponseWriter, r *http.Request, tenantSlug, providerKey string, profile oauth.Profile) {
	...
	err := h.dbOf(ctx).QueryRow(ctx, `
		...
		WHERE s.provider = $1 AND s.provider_user_id = $2 AND u.status = 'active'
	...
	if err != nil {
		...
		`, userID, providerKey, profile.ProviderUserID, nullString(profile.Email), nullString(profile.Name), nullString(profile.AvatarURL))
		if email != "" {
...
func (h *Handler) createSocialUser(ctx context.Context, email string, profile oauth.Profile) (userID, outEmail, role string, err error) {
	if email == "" {
