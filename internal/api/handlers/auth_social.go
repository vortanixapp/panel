...
func (h *Handler) loginOrRegisterSocial(w http.ResponseWriter, r *http.Request, tenantSlug, providerKey string, profile oauth.Profile) {
	...
	// Второй фактор обходился полностью: аккаунт с паролем и включённой 2FA
	// открывался соцаккаунтом с тем же адресом, минуя код. Ведём соцвход через
	// тот же барьер, что и обычный вход.
	...
	// Соцвход не попадал ни в журнал попыток, ни в уведомление о новом
	// устройстве: захват аккаунта через него оставался незаметным.
