...
func (h *Handler) AdminAnalytics(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]any{
		"days":      days,
		"revenue":   h.analyticsRevenue(ctx, days),
		"series":    h.analyticsSeries(ctx, days),
		"sources":   h.analyticsBySource(ctx, days),
		"recurring": h.analyticsRecurring(ctx),
		"customers": h.analyticsCustomers(ctx, days),
		"churn":     h.analyticsChurn(ctx, days),
		"breakdown": map[string]any{
			"games":     h.analyticsBreakdown(ctx, "game"),
			"locations": h.analyticsBreakdown(ctx, "location"),
			"tariffs":   h.analyticsBreakdown(ctx, "tariff"),
		},
	})
}

func (h *Handler) analyticsRevenue(ctx context.Context, days int) map[string]any {
	var charges, chargesPrev, topups, topupsPrev float64
	var payers int

	_ = h.readerOf(ctx).QueryRow(ctx, `
		SELECT
			COALESCE(SUM(ABS(amount)) FILTER (WHERE created_at >= now() - make_interval(days => $1::int)), 0),
			COALESCE(SUM(ABS(amount)) FILTER (
				WHERE created_at >= now() - make_interval(days => $1::int * 2)
				  AND created_at <  now() - make_interval(days => $1::int)), 0)
		FROM core.transactions
		WHERE type = 'debit'
	`, days).Scan(&charges, &chargesPrev)

	_ = h.readerOf(ctx).QueryRow(ctx, `
		SELECT
			COALESCE(SUM(amount) FILTER (WHERE created_at >= now() - make_interval(days => $1::int)), 0),
			COALESCE(SUM(amount) FILTER (
				WHERE created_at >= now() - make_interval(days => $1::int * 2)
				  AND created_at <  now() - make_interval(days => $1::int)), 0),
			COUNT(DISTINCT user_id) FILTER (WHERE created_at >= now() - make_interval(days => $1::int))
		FROM core.payments
		WHERE status IN ('success', 'succeeded', 'completed', 'paid')
	`, days).Scan(&topups, &topupsPrev, &payers)
...
func (h *Handler) analyticsSeries(ctx context.Context, days int) []map[string]any {
	rows, err := h.readerOf(ctx).Query(ctx, `
		...
		SELECT d.day::date::text,
		       ...
		       COALESCE((
		           ...
		           WHERE p.status IN ('success', 'succeeded', 'completed', 'paid')
...
func (h *Handler) analyticsBySource(ctx context.Context, days int) []map[string]any {
	rows, err := h.readerOf(ctx).Query(ctx, `
...
func (h *Handler) analyticsRecurring(ctx context.Context) map[string]any {
	var monthly float64
	...
	_ = h.readerOf(ctx).QueryRow(ctx, `
		...
		WHERE COALESCE(s.is_blocked, false) = false
		  AND (s.expires_at IS NULL OR s.expires_at > now())
	`).Scan(&monthly, &active, &expiring)
...
func (h *Handler) analyticsCustomers(ctx context.Context, days int) map[string]any {
	var registered, withServer, withPayment, total int
	_ = h.readerOf(ctx).QueryRow(ctx, `
		SELECT
			COUNT(*) FILTER (WHERE u.created_at >= now() - make_interval(days => $1::int)),
			COUNT(*) FILTER (WHERE u.created_at >= now() - make_interval(days => $1::int)
		...
		FROM core.users u
		WHERE u.role = 'user'
	`, days).Scan(&registered, &withServer, &withPayment, &total)
...
func (h *Handler) analyticsChurn(ctx context.Context, days int) map[string]any {
...
func (h *Handler) analyticsBreakdown(ctx context.Context, dimension string) []map[string]any {
	var groupExpr, joinExpr string
	switch dimension {
	case "game":
		groupExpr = "COALESCE(g.name, s.game_id)"
		joinExpr = "LEFT JOIN core.games g ON g.slug = s.game_id"
	case "location":
	...
	rows, err := h.readerOf(ctx).Query(ctx, query)
	if err != nil {
		return []map[string]any{}
	}
	defer rows.Close()
...
func (h *Handler) AdminAnalyticsExport(w http.ResponseWriter, r *http.Request) {
	...
	days := analyticsDays(r)
	series := h.analyticsSeries(r.Context(), days)
...
func (h *Handler) AdminServerAnalytics(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.readerOf(r.Context()).Query(r.Context(), `
		...
		FROM core.transactions
		WHERE source_id = $1::uuid AND type = 'debit'
