...
func (h *Handler) listGames(w http.ResponseWriter, r *http.Request, admin bool) {
	...
	q := `SELECT id::text, slug, name, description, image_url, active FROM core.games WHERE true`
	if !admin {
		q += ` AND active = true`
	}
	rows, err := h.dbOf(r.Context()).Query(r.Context(), q)
	if err != nil {
...
func (h *Handler) ListTariffsPublic(w http.ResponseWriter, r *http.Request) {
	...
	rows, _ := h.dbOf(r.Context()).Query(r.Context(), `
		...
		FROM core.tariffs WHERE active = true
	`)
	defer func() {
		if rows != nil {
			rows.Close()
...
func (h *Handler) CreatePlugin(w http.ResponseWriter, r *http.Request) {
	...
	if id == "" && slug != "" {
		_ = h.dbOf(r.Context()).QueryRow(r.Context(),
			`SELECT id::text FROM core.plugins WHERE slug=$1`, slug).Scan(&id)
	}
	if id != "" {
		tag, err := h.dbOf(r.Context()).Exec(r.Context(), `
			UPDATE core.plugins
			SET name = $2, category = NULLIF($3, ''), version = NULLIF($4, ''),
			    description = NULLIF($5, ''), install_path = $6, supported_games = $7,
			    file_actions = $8::jsonb, uninstall_actions = $9::jsonb,
			    restart_required = $10, active = $11, updated_at = now()
			WHERE id = $1::uuid
	...
	var newID string
	err = h.dbOf(r.Context()).QueryRow(r.Context(), `
		INSERT INTO core.plugins ( slug, name, category, version, description,
...
func (h *Handler) CreateMap(w http.ResponseWriter, r *http.Request) {
	...
	if id == "" && slug != "" {
		_ = h.dbOf(r.Context()).QueryRow(r.Context(),
			`SELECT id::text FROM core.maps WHERE slug=$1`, slug).Scan(&id)
	}
	if id != "" {
		tag, err := h.dbOf(r.Context()).Exec(r.Context(), `
			UPDATE core.maps
			SET name = $2, category = NULLIF($3, ''), version = NULLIF($4, ''),
			    game_id = $5::uuid, restart_required = $6, active = $7
			WHERE id = $1::uuid
	...
	var newID string
	err := h.dbOf(r.Context()).QueryRow(r.Context(), `
		INSERT INTO core.maps ( slug, name, category, version, game_id, restart_required, active)
		VALUES ( $1, $2, NULLIF($3, ''), NULLIF($4, ''), $5::uuid, $6, $7)
		RETURNING id::text
