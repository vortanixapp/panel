...
func (h *Handler) ListPlugins(w http.ResponseWriter, r *http.Request) {
	...
	// Отдаём все поля каталога, а не четыре: описание, категория, путь
	// установки, поддерживаемые игры и действия с файлами — это и есть плагин,
	// без них форма редактирования показывать нечего.
...
// jsonOrEmptyArray разворачивает JSONB-колонку в значение для ответа.
...
// pluginImageURL собирает адрес превью. Пустой путь — нет картинки; адрес
// строится через ту же ручку, что и у старой панели, чтобы файл не отдавался
...
// normalizeInstallPath повторяет разбор из старой панели: пробелы и слэши по
// краям срезаются, '..' запрещён. Пустой путь допустим — бывают плагины,
// состоящие из одних только действий с файлами.
...
// normalizeSupportedGames приводит список игр к виду, который понимает выдача
// каталога. Пустой список означает «все игры», и старая панель записывала для
// этого случая ['*'] — оба варианта считаем одним и тем же.
...
// pluginActionKinds — набор действий с файлами из старой панели. Неизвестное
// действие там молча становилось ensure_contains, и это поведение сохраняем:
// иначе перенесённые записи начали бы отваливаться с ошибкой.
...
// normalizePluginActions собирает действия с файлами по правилам старой панели.
//
// Строки без пути или с '..' пропускаются молча; для replace_regex обязателен
// pattern, для остальных — непустой список строк. Пустой результат — пустой
// массив, а не NULL: колонка объявлена NOT NULL DEFAULT '[]'.
...
// truthyFlag: старая панель считала галочку установленной только для '1',
// 'true' и 'on'.
...
// splitActionLines принимает как список строк, так и многострочный текст из
// формы — старая панель резала его по \r\n, \r и \n и выбрасывала пустые.
...
func (h *Handler) ListMaps(w http.ResponseWriter, r *http.Request) {
	...
	// Игра у карты хранится ссылкой game_id на core.games, а не слагом:
	// колонки game_slug в core.maps нет и не было. Запрос к ней падал, ошибку
	// глушило `rows, _ :=`, и раздел «Карты» в админке всегда показывал пустой
	// список — при живых записях в базе.
	rows, err := h.dbOf(r.Context()).Query(r.Context(), `
...
func (h *Handler) CreateMap(w http.ResponseWriter, r *http.Request) {
	...
	// Игру приводим к её id: колонки game_slug в core.maps нет, есть ссылка на
	// core.games. Неизвестный слаг — не молчаливый пропуск, а понятная ошибка.
	var gameID *string
	if gameSlug != "" {
		var found string
		err := h.dbOf(r.Context()).QueryRow(r.Context(),
