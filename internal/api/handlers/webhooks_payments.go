...
func (h *Handler) PayPalWebhook(w http.ResponseWriter, r *http.Request) {
	...
	// Сумму берём оттуда, где её прислал PayPal: в захвате она в resource,
	// в одобрении заказа — в единице покупки.
...
func (h *Handler) NowPaymentsWebhook(w http.ResponseWriter, r *http.Request) {
	...
	// Недоплату касса помечает своим статусом partially_paid, до сюда она не
	// доходит. Сверка всё равно нужна: она ловит расхождение между заказом в
	// панели и заказом, заведённым в кассе.
