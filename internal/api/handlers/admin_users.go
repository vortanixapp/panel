...
func (h *Handler) adminUserCounts(ctx context.Context) map[string]int {
	...
	_ = h.dbOf(ctx).QueryRow(ctx, `
		...
		FROM core.users
	`).Scan(&total, &admins, &support, &users, &active, &unverified, &blocked)
...
func (h *Handler) GetUser(w http.ResponseWriter, r *http.Request) {
	...
	detail, err := h.adminUserDetail(r.Context(), id)
	if err != nil {
...
func (h *Handler) ToggleUserBlock(w http.ResponseWriter, r *http.Request) {
	...
	_, err = h.dbOf(r.Context()).Exec(r.Context(), `
		UPDATE core.users SET status = $2 WHERE id = $1
	`, id, newStatus)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "database error")
		return
...
func (h *Handler) adjustWalletBalance(
	ctx context.Context, walletID, adminID, userID, currency string, newBalance float64,
) error {
	var current float64
	if err := h.dbOf(ctx).QueryRow(ctx, `
		SELECT balance::float8 FROM core.wallets WHERE id = $1 AND user_id = $2
	`, walletID, userID).Scan(&current); err != nil {
		return err
	}
	...
	if diff > 0 {
		...
		if _, err := tx.Exec(ctx, `
			INSERT INTO core.transactions ( wallet_id, type, amount, description, meta, source_type, source_id)
			VALUES ( $1, 'credit', $2, $3, $4::jsonb, 'admin_user', $5::uuid)
		`, walletID, diff, "Корректировка баланса админом", fmt.Sprintf(
		...
		if _, err := tx.Exec(ctx, `
			INSERT INTO core.transactions ( wallet_id, type, amount, description, meta, source_type, source_id)
			VALUES ( $1, 'debit', $2, $3, $4::jsonb, 'admin_user', $5::uuid)
		`, walletID, amount, "Корректировка баланса админом", fmt.Sprintf(
			`{"action":"admin_wallet_balance_set","admin_id":"%s","user_id":"%s","currency":"%s","old_balance":%f,"new_balance":%f}`,
			adminID, userID, currency, current, newBalance,
		), userID); err != nil {
			return err
...
func (h *Handler) UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	if status, ok := body["status"].(string); ok && status != "" {
		if status == "active" || status == "disabled" {
			_, _ = h.dbOf(ctx).Exec(ctx, `UPDATE core.users SET status = $2 WHERE id = $1 AND role != 'owner'`, id, status)
		}
	}
	...
	if strings.TrimSpace(email) != "" {
		_, err := h.dbOf(ctx).Exec(ctx, `
			UPDATE core.users SET email = $2 WHERE id = $1
	...
	if pw, ok := body["password"].(string); ok && strings.TrimSpace(pw) != "" {
		...
		if err == nil {
			_, _ = h.dbOf(ctx).Exec(ctx, `UPDATE core.users SET password_hash = $2 WHERE id = $1`, id, string(hash))
		}
	}
	...
	if wallets, ok := body["wallets"].(map[string]any); ok {
		for _, currency := range []string{"RUB", "USD", "EUR"} {
			...
			var walletID string
			err := h.dbOf(ctx).QueryRow(ctx, `
				SELECT id::text FROM core.wallets WHERE user_id = $1 AND UPPER(currency) = $2
			`, id, currency).Scan(&walletID)
			if err != nil {
				continue
			}
			_ = h.adjustWalletBalance(ctx, walletID, claims.UserID, id, currency, newBal)
		}
	}
