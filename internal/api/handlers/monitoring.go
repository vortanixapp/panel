...
func (h *Handler) MonitoringIndex(w http.ResponseWriter, r *http.Request) {
	...
	_ = h.readerOf(ctx).QueryRow(ctx, `
		...
		WHERE i.started_at >= now() - INTERVAL '7 days'
		  AND ($1 OR s.user_id = $2)
	`, isStaffRole(claims.Role), claims.UserID).Scan(&incidents7d)
...
func (h *Handler) MonitoringShow(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := h.resolveServerAccess(ctx, claims.UserID, claims.Role, serverID); err != nil {
		writeError(w, http.StatusNotFound, "server not found")
		return
	}

	rows, err := h.loadMonitoringRows(ctx, monitoringScope{
		ServerID: serverID, AnyOwner: true,
	})
	...
	if row.Status == "running" {
		if res, ok := h.monitoringGameQuery(ctx, serverID, row); ok {
...
func (h *Handler) monitoringGameQuery(
	ctx context.Context, serverID string, row monitoringRow,
) (map[string]any, bool) {
...
func (h *Handler) loadMonitoringSettings(ctx context.Context, serverID string) monitoringSettings {
...
func (h *Handler) MonitoringSettingsUpdate(w http.ResponseWriter, r *http.Request) {
	...
	current := h.loadMonitoringSettings(ctx, serverID)
	var body struct {
		PublicEnabled *bool     `json:"public_enabled"`
		Title         *string   `json:"title"`
		Description   *string   `json:"description"`
	...
	_, err = h.dbOf(ctx).Exec(ctx, `
		INSERT INTO core.server_monitoring
		    (server_id, public_enabled, title, description, tags,
		...
		ON CONFLICT (server_id) DO UPDATE SET
		    ...
		    updated_at     = now()
	`, serverID, current.PublicEnabled, current.Title, current.Description,
...
func (h *Handler) MonitoringTop(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	if !h.monitoringAllowed(w, r) {
		return
	}

	rows, err := h.loadMonitoringRows(ctx, monitoringScope{
		AnyOwner: true, PublicOnly: true, Limit: 200,
	})
	if err != nil {
		writeError(w, http.StatusInternalServerError, "database error")
		return
...
func (h *Handler) monitoringAllowed(w http.ResponseWriter, r *http.Request) bool {
...
func (h *Handler) MonitoringPublic(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.loadMonitoringRows(ctx, monitoringScope{
		AnyOwner: true, ServerID: serverID,
	})
	if err != nil || len(rows) == 0 {
		writeError(w, http.StatusNotFound, "server not found")
		return
...
func (h *Handler) MonitoringPublicVote(w http.ResponseWriter, r *http.Request) {
	...
	if h.readerOf(ctx).QueryRow(ctx, `
		SELECT public_enabled FROM core.server_monitoring WHERE server_id = $1
	`, serverID).Scan(&enabled) != nil || !enabled {
		writeError(w, http.StatusNotFound, "public page disabled")
		return
	}
