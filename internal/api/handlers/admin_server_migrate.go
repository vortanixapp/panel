...
func (h *Handler) AdminServerMigrationTargets(w http.ResponseWriter, r *http.Request) {
	...
	if h.readerOf(ctx).QueryRow(ctx, `
		SELECT node_id::text, game_id, limits FROM core.servers WHERE id = $1 AND tenant_id = $2
	`, serverID, claims.TenantID).Scan(&currentNode, &gameID, &limits) != nil {
...
func (h *Handler) AdminServerMigrate(w http.ResponseWriter, r *http.Request) {
	...
	`, serverID, claims.TenantID).Scan(&fromNode, &serverName, &provStatus, &serverStatus) != nil {
		writeError(w, http.StatusNotFound, "server not found")
		return
	}
	if fromNode == toNode {
	...
	if h.dbOf(ctx).QueryRow(ctx, `SELECT name, status FROM core.nodes WHERE id = $1 AND tenant_id = $2`,
		toNode, claims.TenantID).Scan(&toName, &toStatus) != nil {
	...
	if h.nodeInMaintenance(ctx, claims.TenantID, toNode) {
	...
	err := h.dbOf(ctx).QueryRow(ctx, `
		INSERT INTO core.server_migrations
			(tenant_id, server_id, from_node_id, to_node_id, from_node_name, to_node_name,
			 status, stage, remove_source, created_by)
		VALUES ($1, $2, $3, $4, $5, $6, 'pending', 'queued', $7, $8)
		RETURNING id::text
	`, claims.TenantID, serverID, fromNode, toNode, fromName, toName,
	...
	if _, err := h.dbOf(ctx).Exec(ctx, `
		...
		VALUES ($1, 'migrate_server', 'pending', $2::jsonb)
	...
	audit(ctx, h.dbOf(ctx), claims.TenantID, claims.UserID, "server.migrate", "server:"+serverID,
		map[string]any{"from": fromName, "to": toName, "remove_source": removeSource})
	h.auditAlert(ctx, claims.TenantID, claims.UserID, claims.Email, "server.migrate",
