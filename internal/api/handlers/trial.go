...
func (h *Handler) trialSettings(ctx context.Context) trialSettings {
	s := trialSettings{
		Hours:        intSetting(h.tenantSettingString(ctx, "trial.hours"), 2),
		CooldownDays: intSetting(h.tenantSettingString(ctx, "trial.cooldown_days"), 30),
		MemoryMB:     intSetting(h.tenantSettingString(ctx, "trial.memory_mb"), 0),
		DiskMB:       intSetting(h.tenantSettingString(ctx, "trial.disk_mb"), 0),
	}
	s.Enabled = truthySetting(h.tenantSettingString(ctx, "trial.enabled"))
	for _, g := range strings.Split(h.tenantSettingString(ctx, "trial.games"), ",") {
...
func (h *Handler) trialAvailableAt(ctx context.Context, userID string, cooldownDays int) time.Time {
	var last time.Time
...
func (h *Handler) TrialCreate(w http.ResponseWriter, r *http.Request) {
	...
	if nodeID == "" {
		var err error
		nodeID, err = h.pickTrialNode(ctx)
		if err != nil {
	...
	err := h.dbOf(ctx).QueryRow(ctx, `
		INSERT INTO core.servers
			( node_id, game_id, name, limits, user_id, provisioning_status, expires_at, is_trial, rental_period_days, config)
		VALUES ( $1::uuid, $2, $3, $4::jsonb, $5::uuid, 'provisioning', $6, true, 1,
		        jsonb_build_object('startup_params', $7::text))
		RETURNING id::text
...
func (h *Handler) pickTrialNode(ctx context.Context) (string, error) {
