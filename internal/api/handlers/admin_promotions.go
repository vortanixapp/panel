...
type promotionBody struct {
	...
	// К чему применяется акция: аренда, продление, пополнение. Колонка была с
	// самого начала и читалась движком подбора, но принять её было неоткуда —
	// все акции создавались с пустым списком, то есть действовали сразу на всё.
	AppliesTo *[]string `json:"applies_to"`

	// Бонус при пополнении. Свои колонки, отдельные от скидки: скидка
	// уменьшает цену покупки, бонус увеличивает зачисление на баланс.
	BonusPercent *float64 `json:"bonus_percent"`
	BonusFixed   *float64 `json:"bonus_fixed"`

	// Ограничения. Движок читает их из filters, а записать было нечем.
...
func parseDate(v *string) (any, bool) {
	...
	// Форма отдаёт «ГГГГ-ММ-ДДTЧЧ:ММ»: акция «до 18:00 сегодня» без времени
	// не настраивается, а раньше в интерфейсе стоял выбор только даты.
...
// normalizeDiscountType допускает отсутствие скидки.
//
// Раньше пустое значение приводилось к percent, и акция «только бонус на
// пополнение, без скидки» была невыразима: она превращалась в скидку 0%,
// которую движок подбора считал за настоящую.
...
// normalizePromoCode приводит код к верхнему регистру и превращает пустой в
// NULL.
//
// Без этого «promo» и «PROMO» ложились как две разные записи, хотя ищутся они
// по UPPER(code) — и находилась из них случайная. А пустая строка вместо NULL
// не давала завести вторую безкодовую акцию: для уникальности пустая строка —
// такое же значение, как любое другое, а NULL с NULL не сравнивается.
...
// normalizeAppliesTo оставляет только известные сценарии. Пустой список
// означает «применяется везде» — так его и понимает движок подбора.
...
// promoFiltersPatch собирает то, что нужно дописать в filters. Возвращает nil,
// если ни одно ограничение не пришло: тогда прежние значения не трогаются.
