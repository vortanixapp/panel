...
func (h *Handler) recordLoginAttempt(ctx context.Context, r *http.Request, tenantID, userID, email, reason string, success bool) {
	agent := r.Header.Get("User-Agent")
	if len(agent) > 300 {
		agent = agent[:300]
	}
	_, _ = h.dbOf(ctx).Exec(ctx, `
		INSERT INTO core.login_attempts (tenant_id, email, user_id, ip, user_agent, success, reason)
		VALUES (NULLIF($1, '')::uuid, $2, NULLIF($3, '')::uuid, $4, $5, $6, $7)
	`, tenantID, strings.ToLower(strings.TrimSpace(email)), userID, clientIP(r), agent, success, reason)
}
...
func (h *Handler) autoBlockIfNeeded(ctx context.Context, tenantID, ip string) {
	if strings.TrimSpace(ip) == "" || strings.TrimSpace(tenantID) == "" {
		return
	}
	var fails int
	...
	_, _ = h.dbOf(ctx).Exec(ctx, `
		INSERT INTO core.ip_blocks (tenant_id, ip, reason, auto, expires_at)
	...
	`, tenantID, ip, strconv.Itoa(fails)+" неудачных входов подряд", loginAutoBlockFor.String())
}

func (h *Handler) staffRequires2FA(ctx context.Context, tenantID string) bool {
	return truthySetting(h.tenantSettingString(ctx, tenantID, "security.staff_2fa_required"))
}
...
func (h *Handler) AdminLoginAttempts(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.readerOf(r.Context()).Query(r.Context(), `
		...
		WHERE (tenant_id = $1 OR tenant_id IS NULL)
		  AND ($2 = false OR success = false)
		  AND ($3 = '' OR email ILIKE '%' || $3 || '%' OR ip ILIKE '%' || $3 || '%')
		ORDER BY created_at DESC
		LIMIT 200
	`, claims.TenantID, onlyFailed, search)
	if err != nil {
...
func (h *Handler) AdminIPBlockCreate(w http.ResponseWriter, r *http.Request) {
	...
	err := h.dbOf(r.Context()).QueryRow(r.Context(), `
		INSERT INTO core.ip_blocks (tenant_id, ip, reason, auto, expires_at, created_by)
		VALUES ($1, $2, $3, false, $4, $5)
		ON CONFLICT (tenant_id, ip) DO UPDATE
		SET reason = EXCLUDED.reason, auto = false, expires_at = EXCLUDED.expires_at,
		    created_by = EXCLUDED.created_by
		RETURNING id::text
	`, claims.TenantID, ip, strings.TrimSpace(body.Reason), expires, nullableUUID(claims.UserID)).Scan(&id)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "database error")
		return
	}
	audit(r.Context(), h.dbOf(r.Context()), claims.TenantID, claims.UserID, "ip.block", "ip:"+ip,
		map[string]any{"reason": body.Reason, "hours": body.Hours})
	h.auditAlert(r.Context(), claims.TenantID, claims.UserID, claims.Email, "ip.block", ip)
...
func (h *Handler) AdminIPBlockDelete(w http.ResponseWriter, r *http.Request) {
	...
	tag, err := h.dbOf(r.Context()).Exec(r.Context(), `
		DELETE FROM core.ip_blocks WHERE id = $1 AND tenant_id = $2
	`, id, claims.TenantID)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "database error")
		return
	...
	audit(r.Context(), h.dbOf(r.Context()), claims.TenantID, claims.UserID, "ip.unblock", "ip_block:"+id, nil)
