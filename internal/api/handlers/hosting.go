...
func (h *Handler) AdminHostingServers(w http.ResponseWriter, r *http.Request) {
	...
	rows, _ := h.dbOf(r.Context()).Query(r.Context(), `SELECT id::text, name, panel_type, hostname, active FROM core.hosting_servers`)
	defer func() {
		if rows != nil {
			rows.Close()
		}
...
func (h *Handler) AdminHostingPlans(w http.ResponseWriter, r *http.Request) {
	...
	rows, _ := h.dbOf(r.Context()).Query(r.Context(), `
		...
		FROM core.hosting_plans ORDER BY position, name
	`)
	defer func() {
		if rows != nil {
			rows.Close()
