...
func (h *Handler) ImpersonateUser(w http.ResponseWriter, r *http.Request) {
	...
	err := h.dbOf(r.Context()).QueryRow(r.Context(), `SELECT email, role FROM core.users WHERE id = $1`, id).Scan(&email, &role)
...
func (h *Handler) userPermissions(ctx context.Context, role string) map[string]bool {
	return h.rbacLoadRolePermissions(ctx, rbacNormalizeRole(role))
}
