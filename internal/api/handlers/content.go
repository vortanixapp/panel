...
func (h *Handler) ListSupportTickets(w http.ResponseWriter, r *http.Request) {
	...
	q := `SELECT t.id::text, t.subject, t.status, t.created_at::text, t.category, t.priority,
	      ...
	      FROM core.support_tickets t WHERE true`
	args := []any{}
	...
	if labels := h.serviceLabels(r.Context(), serviceIDs); len(labels) > 0 {
		for _, item := range list {
...
func (h *Handler) CreateSupportTicket(w http.ResponseWriter, r *http.Request) {
	...
	_ = h.dbOf(r.Context()).QueryRow(r.Context(), `
		INSERT INTO core.support_tickets
		    ( user_id, subject, category, priority, service_kind, service_id)
		VALUES ( $1, $2, $3, $4, $5, NULLIF($6, '')::uuid) RETURNING id::text
	`, claims.UserID, body.Subject, category, priority,
...
func (h *Handler) GetSupportTicket(w http.ResponseWriter, r *http.Request) {
	...
	err := h.dbOf(ctx).QueryRow(ctx, `
		SELECT user_id::text FROM core.support_tickets WHERE id = $1
	`, id).Scan(&ticketUserID)
	if err != nil {
	...
	if rows != nil {
		for rows.Next() {
			...
			if rows.Scan(&mid, &body, &staff, &created, &metaRaw) == nil {
				...
				list := h.supportAttachmentsFor(ctx, id, mid)
...
func (h *Handler) ReplySupportTicket(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.dbOf(ctx).QueryRow(ctx, `
		SELECT user_id::text FROM core.support_tickets WHERE id = $1
	`, ticketID).Scan(&ticketUserID); err != nil {
		writeError(w, http.StatusNotFound, "not found")
		return
	}
...
func (h *Handler) CloseSupportTicket(w http.ResponseWriter, r *http.Request) {
	...
	h.notifySupportStatus(ctx, ticketID, "closed")
...
func (h *Handler) ListNews(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.dbOf(r.Context()).Query(r.Context(), `
		...
		FROM core.news n
		CROSS JOIN LATERAL (
		    SELECT news_read_at FROM core.users WHERE id = $1::uuid
		) u
		WHERE n.active = true AND n.published_at IS NOT NULL
...
func (h *Handler) GetNews(w http.ResponseWriter, r *http.Request) {
	...
	err := h.dbOf(r.Context()).QueryRow(r.Context(), `
		...
		FROM core.news WHERE slug = $1 AND active = true
	`, slug).Scan(&id, &title, &excerpt, &body, &pub, &tag, &pinned)
	if err != nil {
		writeError(w, http.StatusNotFound, "not found")
		return
	}
	...
	imgs := h.newsImages(r, []string{id})[id]
	if imgs == nil {
...
func (h *Handler) CreateNews(w http.ResponseWriter, r *http.Request) {
	...
	var id string
	err = h.dbOf(r.Context()).QueryRow(r.Context(), `
		INSERT INTO core.news ( slug, title, excerpt, body, published_at, active, tag, pinned)
		VALUES ( $1, $2, NULLIF($3, ''), $4,
		        CASE WHEN $5::bool THEN $6::timestamptz ELSE now() END,
		        COALESCE($7, true), COALESCE(NULLIF($8, ''), 'update'), COALESCE($9, false))
		RETURNING id::text
...
func (h *Handler) MonitoringPublicLive(w http.ResponseWriter, r *http.Request) {
	...
	if !h.monitoringPublicAllowed(w, ctx, r) {
		return
	}
	if !h.publicMonitoringAllowed(w, ctx, serverID) {
		return
	}
	if _, _, _, _, err := h.loadMonitoringServer(ctx, serverID); err != nil {
...
func (h *Handler) publicMonitoringAllowed(w http.ResponseWriter, ctx context.Context, serverID string) bool {
	if !h.loadMonitoringSettings(ctx, serverID).PublicEnabled {
...
func (h *Handler) loadMonitoringServer(ctx context.Context, serverID string) (name, status, gameID, gameName string, err error) {
	err = h.dbOf(ctx).QueryRow(ctx, `
		SELECT s.name, s.status, s.game_id, COALESCE(g.name, '')
		FROM core.servers s
		LEFT JOIN core.games g ON g.slug = s.game_id
		WHERE s.id = $1
	`, serverID).Scan(&name, &status, &gameID, &gameName)
	return
}
...
func (h *Handler) ListMailings(w http.ResponseWriter, r *http.Request) {
	...
	rows, _ := h.dbOf(r.Context()).Query(r.Context(), `SELECT id::text, subject, status FROM core.mailings`)
	defer func() {
		if rows != nil {
			rows.Close()
		}
...
func (h *Handler) CreateMailing(w http.ResponseWriter, r *http.Request) {
	...
	if id, ok := body["id"].(string); ok && id != "" {
		_, _ = h.dbOf(r.Context()).Exec(r.Context(), `
			UPDATE core.mailings SET title=$2, subject=$3, body=$4 WHERE id=$1::uuid
		`, id, title, subject, bodyText)
		writeJSON(w, http.StatusOK, map[string]string{"status": "updated"})
		return
	}
	var id string
	_ = h.dbOf(r.Context()).QueryRow(r.Context(), `
		INSERT INTO core.mailings ( title, subject, body) VALUES ( COALESCE($1, $2), $2, $3) RETURNING id::text
	`, title, subject, bodyText).Scan(&id)
	writeJSON(w, http.StatusCreated, map[string]string{"id": id})
}
...
func (h *Handler) SendMailing(w http.ResponseWriter, r *http.Request) {
	...
	_, err := h.dbOf(r.Context()).Exec(r.Context(), `
		UPDATE core.mailings SET status = 'queued', scheduled_at = now() WHERE id = $1
	`, mailingID)
	if err != nil {
		writeError(w, http.StatusNotFound, "not found")
		return
	...
	_, _ = h.dbOf(r.Context()).Exec(r.Context(), `
		INSERT INTO core.jobs ( type, status, payload) VALUES ( 'send_mailing', 'pending', $1::jsonb)
	`, payload)
...
func (h *Handler) MonitoringPublicBanner(w http.ResponseWriter, r *http.Request) {
	...
	name, status, _, _, err := h.loadMonitoringServer(ctx, serverID)
	if err != nil {
		writeError(w, http.StatusNotFound, "not found")
		return
	}
