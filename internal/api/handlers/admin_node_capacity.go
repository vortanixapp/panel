...
type nodeCapacity struct {
	...
	ReservedRAM  int
	MetricsKnown bool
	// DiskQuota — работают ли квоты на локации. nil означает «агент ещё не
	// отчитался»: это не то же самое, что «квот нет».
	DiskQuota *bool
}
...
func (h *Handler) loadNodeCapacity(ctx context.Context, tenantID, nodeID string) (*nodeCapacity, error) {
	...
	// Работают ли квоты на этой локации. Признак присылает агент в heartbeat:
	// это свойство ноды, а не настройка, и узнать его можно только оттуда.
	// Пока агент не отчитался, поле остаётся неизвестным — показывать «нет»
	// значило бы обвинить локацию в том, чего мы ещё не проверяли.
...
// boolPtrFromMetric переводит числовую метрику в признак. nil означает «агент
// ещё не отчитался»: это не то же самое, что «квот нет», и слипаться в одно
// значение эти два случая не должны — иначе новая локация выглядела бы
// неисправной с первой секунды.
