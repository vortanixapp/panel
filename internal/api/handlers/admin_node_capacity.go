...
func (h *Handler) loadNodeCapacity(ctx context.Context, nodeID string) (*nodeCapacity, error) {
	...
	`, nodeID).Scan(&c.NodeName, &maxServers, &overcommit, &reserved)
	if err != nil {
		return nil, err
	}
	...
	rows, err := h.readerOf(ctx).Query(ctx, `
		SELECT limits FROM core.servers WHERE node_id = $1
	`, nodeID)
	if err != nil {
		return nil, err
	}
...
func (h *Handler) AdminNodeCapacity(w http.ResponseWriter, r *http.Request) {
	...
	c, err := h.loadNodeCapacity(ctx, nodeID)
	if err != nil {
...
func (h *Handler) AdminNodeCapacityUpdate(w http.ResponseWriter, r *http.Request) {
	...
	tag, err := h.dbOf(r.Context()).Exec(r.Context(), `
		UPDATE core.nodes SET
			max_servers     = CASE WHEN $2::bool THEN $3::int ELSE max_servers END,
			ram_overcommit  = COALESCE($4, ram_overcommit),
			reserved_ram_mb = COALESCE($5, reserved_ram_mb)
		WHERE id = $1
	`, nodeID,
	...
	audit(r.Context(), h.dbOf(r.Context()), claims.UserID, "node.capacity", "node:"+nodeID, nil)
	writeJSON(w, http.StatusOK, map[string]string{"status": "updated"})
}

func (h *Handler) nodeCapacityReason(ctx context.Context, nodeID, gameID string) string {
	c, err := h.loadNodeCapacity(ctx, nodeID)
	if err != nil {
		return ""
	}
