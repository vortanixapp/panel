...
func (h *Handler) ServerPluginsList(w http.ResponseWriter, r *http.Request) {
	...
	// Список показывает, что установлено на конкретном сервере. Изменения
	// плагинов проверку владельца проходили, а чтение — нет: состав плагинов
	// чужого сервера читался по одному его id.
	if !h.authorizeServerAction(w, r, claims, serverID, "settings_read") {
		return
	}
	gameSlug := h.serverGameSlug(r, claims.TenantID, serverID)
	rows, err := h.dbOf(r.Context()).Query(r.Context(), `
		...
		FROM core.plugins p
		LEFT JOIN core.server_plugins sp ON sp.plugin_id = p.id AND sp.server_id = $2::uuid
		WHERE p.tenant_id = $1 AND p.active = true
		  AND ($3 = '' OR $3 = ANY(p.supported_games) OR cardinality(p.supported_games) = 0)
		ORDER BY p.name ASC
	`, claims.TenantID, serverID, gameSlug)
	if err != nil {
...
func (h *Handler) ServerPluginToggle(w http.ResponseWriter, r *http.Request) {
	...
	state, err := h.loadServerOperableState(r.Context(), claims.TenantID, serverID)
	if err != nil {
	...
	// Переключатель меняет ТОЛЬКО флаг enabled и только у уже установленного
	// плагина. Раньше он вдобавок ставил installed = true и слал агенту
	// plugins_apply без действия — а тот на пустое действие переприменял
	// file_actions либо uninstall_actions. То есть выключение плагина
	// откатывало конфиги, а включение «устанавливало» никогда не установленный
	// плагин, ничего при этом не распаковав.
...
func (h *Handler) serverPluginMutate(w http.ResponseWriter, r *http.Request, installed, enabled bool) {
	...
	// Права проверяем до всего остального: без этого вызова право
	// can_settings_edit не спрашивается вовсе.
	if !h.authorizeServerAction(w, r, claims, serverID, "plugins_apply") {
		return
	}
	...
	spec, err := h.loadPluginInstallSpec(r.Context(), claims.TenantID, pluginID)
	if err != nil {
	...
	payload := h.pluginAgentPayload(r, claims.TenantID, serverID, pluginID)
	payload["action"] = action

	// Перед установкой архив должен оказаться в кэше локации: распаковка идёт
	// на ноде. При удалении он не нужен — там работают только uninstall_actions
	// и список файлов.
	if action == "install" {
		cachePath, delErr := h.deliverPluginArchive(w, r, claims.TenantID, serverID, pluginID)
		if delErr != nil {
	...
	// Состояние пишем ПОСЛЕ агента. Раньше строка помечалась installed=true до
	// вызова и не откатывалась при его ошибке: клиент видел «Установлен» у
	// плагина, которого на сервере нет, и причину узнать было негде — колонка
	// last_error существовала, но никогда не заполнялась.
...
// recordPluginError сохраняет причину сбоя в строке сервера. Колонка last_error
// была заведена с самого начала, но не заполнялась ничем — клиент видел, что
// установка не удалась, и не видел, почему.
...
// deliverPluginArchive доставляет архив плагина на ноду сервера и сам отвечает
// клиенту, если доставка не удалась: ставить плагин без архива бессмысленно,
// а молчаливый пропуск оставил бы сервер в состоянии «установлено», но пусто.
func (h *Handler) deliverPluginArchive(w http.ResponseWriter, r *http.Request, tenantID, serverID, pluginID string) (string, error) {
	nodeID, err := h.serverNodeID(r.Context(), tenantID, serverID)
	if err != nil {
		writeError(w, http.StatusNotFound, "сервер не найден")
		return "", err
	}
	cachePath, err := h.deliverCatalogArchive(r, tenantID, nodeID, catalogPlugins, pluginID)
	if err != nil {
...
func (h *Handler) pluginAgentPayload(r *http.Request, tenantID, serverID, pluginID string) map[string]any {
	payload := map[string]any{"plugin_id": pluginID}
	gameSlug := h.serverGameSlug(r, tenantID, serverID)
	if gameSlug != "" {
	...
	_ = h.dbOf(r.Context()).QueryRow(r.Context(), `
		...
		FROM core.plugins WHERE id = $1::uuid AND tenant_id = $2
	`, pluginID, tenantID).Scan(&installPath, &archivePath, &archiveType, &fileActions, &meta)
	if installPath != "" {
		payload["install_path"] = installPath
	}
	// archive_cache_path заполняет доставка (deliverCatalogArchive): это путь в
	// кэше ноды, а не путь в хранилище панели. Раньше сюда клался archive_path,
	// и агент искал его у себя в кэше, куда этот файл никогда не попадал —
	// установка плагина с архивом падала с «archive not found».
	if archivePath != "" {
...
func (h *Handler) ServerMapsList(w http.ResponseWriter, r *http.Request) {
	...
	serverID := chi.URLParam(r, "id")
	// То же, что и со списком плагинов: какие карты стоят на сервере — это
	// состояние сервера, а не общий каталог.
	if !h.authorizeServerAction(w, r, claims, serverID, "settings_read") {
		return
	}
	gameSlug := h.serverGameSlug(r, claims.TenantID, serverID)
	rows, err := h.dbOf(r.Context()).Query(r.Context(), `
		...
		LEFT JOIN core.server_maps sm ON sm.map_id = m.id AND sm.server_id = $2::uuid
		WHERE m.tenant_id = $1 AND m.active = true
		  AND ($3 = '' OR g.slug = $3 OR m.game_id IS NULL)
		ORDER BY m.name ASC
	`, claims.TenantID, serverID, gameSlug)
	if err != nil {
		writeJSON(w, http.StatusOK, map[string]any{"items": []any{}})
...
func (h *Handler) ServerMapActivate(w http.ResponseWriter, r *http.Request) {
	...
	// Активировать можно только установленную карту: иначе сервер получал бы
	// команду сменить карту на ту, файлов которой у него нет, и падал при
	// старте. Раньше активация сама помечала карту установленной.
	var installed bool
	if err := h.dbOf(r.Context()).QueryRow(r.Context(), `
	...
	payload := h.mapAgentPayload(r, claims.TenantID, serverID, mapID)
	...
	// Флаг активной карты переставляем после успеха агента и одной операцией:
	// снять со всех и поставить этой.
...
func (h *Handler) serverMapMutate(w http.ResponseWriter, r *http.Request, installed, isActive bool) {
	...
	spec, err := h.loadMapInstallSpec(r.Context(), claims.TenantID, mapID)
	if err != nil {
	...
	// Перед установкой архив карты должен лежать в кэше локации. При удалении
	// он не нужен: файлы карты снимаются по списку file_list.
	nodeID, err := h.serverNodeID(r.Context(), claims.TenantID, serverID)
	if err != nil {
		writeError(w, http.StatusNotFound, "сервер не найден")
		return
	...
	if action == "install" {
		cachePath, delErr := h.deliverCatalogArchive(r, claims.TenantID, nodeID, catalogMaps, mapID)
		if delErr != nil {
	...
	// Как и у плагинов, состояние пишем после агента: иначе карта числится
	// установленной, а на сервере её нет.
	if _, err := h.agentCommand(r.Context(), nodeID, serverID, "maps_apply", payload); err != nil {
		code, msg := explainAgentError("maps_apply", err)
		h.recordMapError(r.Context(), serverID, mapID, msg)
		writeCodedError(w, http.StatusBadGateway, code, msg)
...
func (h *Handler) mapAgentPayload(r *http.Request, tenantID, serverID, mapID string) map[string]any {
	...
	_ = h.dbOf(r.Context()).QueryRow(r.Context(), `
		...
		FROM core.maps WHERE id = $1::uuid AND tenant_id = $2
	`, mapID, tenantID).Scan(&slug, &archivePath, &fileList, &meta)
	if slug != "" {
		payload["slug"] = slug
		payload["map_name"] = slug
	}
	// archive_cache_path проставляет доставка: здесь путь в хранилище панели, а
	// агенту нужен путь в кэше его собственной ноды.
	if archivePath != "" {
		payload["has_archive"] = true
	}
	if len(fileList) > 0 {
...
func (h *Handler) nodeMysqlInstances(r *http.Request, tenantID, serverID string) []any {
	var nodeMeta []byte
	_ = h.dbOf(r.Context()).QueryRow(r.Context(), `
	...
	`, serverID, tenantID).Scan(&nodeMeta)
	meta := map[string]any{}
...
func (h *Handler) serverGameSlug(r *http.Request, tenantID, serverID string) string {
	var gameID string
	_ = h.dbOf(r.Context()).QueryRow(r.Context(), `
		SELECT COALESCE(game_id, '') FROM core.servers WHERE id = $1 AND tenant_id = $2
	`, serverID, tenantID).Scan(&gameID)
	return gameID
}
