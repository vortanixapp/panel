...
func agentImage(version string) string {
	image := envOr("AGENT_IMAGE", defaultAgentImage)
...
if [ ! -f /etc/docker/daemon.json ]; then
  ...
  echo "зеркало Docker Hub включено"
elif grep -q registry-mirrors /etc/docker/daemon.json; then
  echo "зеркало Docker Hub уже настроено"
else
  echo "ВНИМАНИЕ: /etc/docker/daemon.json есть, зеркало в нём не задано."
  echo "Без зеркала сборка упирается в лимит Docker Hub на анонимные загрузки."
  echo "Добавьте в файл: \"registry-mirrors\": [\"https://mirror.gcr.io\"] и перезапустите docker."
fi`
}

func dockerCommands() []string {
	return []string{
