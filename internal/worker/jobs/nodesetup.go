...
func (r *Runner) claimNodeSetup(ctx context.Context) (jobID, tenantID string, payload []byte, ok bool) {
	...
	return jobID, tenantID, payload, true
}

func (r *Runner) processNodeSetup(ctx context.Context) bool {
	jobID, tenantID, payload, ok := r.claimNodeSetup(ctx)
	if !ok {
		return false
	}
	...
	node, err := r.loadNodeSSH(ctx, r.db, tenantID, pl.NodeID)
	if err != nil {
		r.failJobGeneric(ctx, r.db, jobID, err.Error())
		r.markSetupFailed(ctx, r.db, tenantID, pl.NodeID, pl.Component, err.Error())
		r.completeSetupProgress(ctx, tenantID, pl.NodeID, "\n❌ "+err.Error()+"\n", pl.Component)
		return true
	}

	relayURL := relayWebsocketURL(envOr("RELAY_PUBLIC_URL", envOr("RELAY_URL", "")))

	progress := r.newSetupProgressWriter(ctx, tenantID, pl.NodeID, pl.Component)
	...
	if pl.Component == "daemon" && relayUnusableForNode(relayURL, node.SSHHost) {
		msg := "у воркера не задан RELAY_PUBLIC_URL: агент на ноде получил бы петлевой адрес " +
			relayURL + " и стучался бы сам в себя. Укажите внешний адрес relay, например wss://relay.vortanix.app"
		r.failNodeSetup(ctx, jobID, tenantID, pl.NodeID, pl.Component, msg, progress)
		return true
	}

	var images []string
	if pl.Component == "images" {
		images = r.gamesToBuild(ctx, tenantID)
	}
	commands, cmdErr := setupCommands(pl.Component, node.Meta, node.AgentToken, node.ID, relayURL, images)
	if cmdErr != nil {
		r.failNodeSetup(ctx, jobID, tenantID, pl.NodeID, pl.Component, cmdErr.Error(), progress)
		return true
	}
	...
	if pl.Component == "images" {
		r.markImagesBuilding(ctx, tenantID, pl.NodeID, images)
	}
	if err := sshclient.Run(cfg, commands, progress); err != nil {
		if pl.Component == "images" {
			r.markImagesResult(ctx, tenantID, pl.NodeID, images, err.Error())
		}
		r.failNodeSetup(ctx, jobID, tenantID, pl.NodeID, pl.Component, err.Error(), progress)
		return true
	}
	if pl.Component == "images" {
		r.markImagesResult(ctx, tenantID, pl.NodeID, images, "")
	}

	fmt.Fprintf(progress, "\n✅ %s installed\n", pl.Component)
	fullLog := progress.finish()
	r.markSetupInstalled(ctx, r.db, tenantID, pl.NodeID, pl.Component)
...
func (r *Runner) loadNodeSSH(ctx context.Context, q dbExec, tenantID, nodeID string) (*nodeSSH, error) {
	var n nodeSSH
	...
	`, nodeID, tenantID).Scan(&n.ID, &n.AgentToken, &sshHost, &sshUser, &n.SSHPort, &sshPassEnc, &meta)
	if err != nil {
...
func (r *Runner) newSetupProgressWriter(ctx context.Context, tenantID, nodeID, component string) *setupProgressWriter {
	...
	if prev := r.currentSetupLog(ctx, tenantID, nodeID); prev != "" {
...
func (w *setupProgressWriter) flush(completed bool) {
	...
	w.mu.Unlock()
	w.r.writeSetupProgress(w.ctx, w.tenantID, w.nodeID, snapshot, completed, w.component)
}
...
func (r *Runner) currentSetupLog(ctx context.Context, tenantID, nodeID string) string {
	var text string
	err := r.db.QueryRow(ctx, `
		SELECT COALESCE(meta -> 'setup_progress' ->> 'log', '')
		FROM core.nodes WHERE id = $1 AND tenant_id = $2
	`, nodeID, tenantID).Scan(&text)
	if err != nil {
		return ""
	}
	return text
...
func (r *Runner) writeSetupProgress(ctx context.Context, tenantID, nodeID, logText string, completed bool, component string) {
	...
	_, _ = r.db.Exec(ctx, `
		UPDATE core.nodes
		SET meta = jsonb_set(COALESCE(meta, '{}'::jsonb), '{setup_progress}', $3::jsonb, true)
		WHERE id = $1 AND tenant_id = $2
	`, nodeID, tenantID, payload)
}

func (r *Runner) completeSetupProgress(ctx context.Context, tenantID, nodeID, logText, component string) {
	prev := r.currentSetupLog(ctx, tenantID, nodeID)
	if prev != "" && !strings.HasSuffix(prev, "\n") {
		prev += "\n"
	}
	r.writeSetupProgress(ctx, tenantID, nodeID, prev+logText, true, component)
}
...
func (r *Runner) setMetaSetupStatus(ctx context.Context, q dbExec, tenantID, nodeID, component, status string) {
	_, _ = q.Exec(ctx, `
		UPDATE core.nodes
		SET meta = COALESCE(meta, '{}'::jsonb) || jsonb_build_object(
			'setup_statuses',
			COALESCE(meta -> 'setup_statuses', '{}'::jsonb) || jsonb_build_object($3::text, $4::text)
		)
		WHERE id = $1 AND tenant_id = $2
	`, nodeID, tenantID, component, status)
}

func (r *Runner) markSetupInstalled(ctx context.Context, q dbExec, tenantID, nodeID, component string) {
	if metaSetupComponent(component) {
		r.setMetaSetupStatus(ctx, q, tenantID, nodeID, component, "installed")
		return
	}
	_, _ = q.Exec(ctx, `
		INSERT INTO core.node_setups (tenant_id, node_id, component, status, installed_at)
	...
	`, tenantID, nodeID, component)
}

func (r *Runner) markSetupFailed(ctx context.Context, q dbExec, tenantID, nodeID, component, errMsg string) {
	if metaSetupComponent(component) {
		r.setMetaSetupStatus(ctx, q, tenantID, nodeID, component, "failed")
		return
	}
	_, _ = q.Exec(ctx, `
		INSERT INTO core.node_setups (tenant_id, node_id, component, status, error_message)
		VALUES ($1, $2, $3, 'failed', $4)
		ON CONFLICT (node_id, component) DO UPDATE SET status = 'failed', error_message = $4, updated_at = now()
	`, tenantID, nodeID, component, errMsg)
}

func (r *Runner) failNodeSetup(ctx context.Context, jobID, tenantID, nodeID, component, errMsg string, progress *setupProgressWriter) {
	fmt.Fprintf(progress, "\n❌ %s\n", errMsg)
	progress.finish()
	r.markSetupFailed(ctx, r.db, tenantID, nodeID, component, errMsg)
...
func (r *Runner) tenantGameImages(ctx context.Context, tenantID string) []string {
...
func (r *Runner) claimJob(ctx context.Context, jobType string) (jobID, tenantID string, payload []byte, ok bool) {
	tx, err := r.db.Begin(ctx)
	if err != nil {
		return "", "", nil, false
	}
	...
	err = tx.QueryRow(ctx, `
		...
		ORDER BY created_at ASC LIMIT 1
		FOR UPDATE SKIP LOCKED
	`, jobType).Scan(&jobID, &tenantID, &payload)
	if err != nil {
		return "", "", nil, false
	}
	if _, err := tx.Exec(ctx, `UPDATE core.jobs SET status = 'running', attempts = attempts + 1 WHERE id = $1`, jobID); err != nil {
...
// licenseKey отдаёт ключ лицензии этого арендатора: он лежит в его базе, а
// воркер с недавних пор ходит по базам арендаторов, так что брать его больше
// неоткуда не нужно.
...
// dockerHubCreds отдаёт учётные данные Docker Hub этого арендатора. Образы игр
// лежат там, и без входа Docker Hub режет анонимные загрузки по IP. Токен
// хранится зашифрованным — тем же ключом, что и остальные секреты панели.
...
// gamesToBuild отдаёт ключи игр, отмеченных к сборке образа. Флаг отдельный от
// «игра подключена»: подключить к продаже и собрать образ на нодах — разные
// решения, и раньше они были слиты в один.
func (r *Runner) gamesToBuild(ctx context.Context, tenantID string) []string {
	rows, err := r.db.Query(ctx, `
		SELECT slug FROM core.games
		WHERE tenant_id = $1 AND build_image = true
		ORDER BY slug
	`, tenantID)
	if err != nil {
		return nil
