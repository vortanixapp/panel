...
func (r *Runner) runDueBackupSchedules(ctx context.Context) {
	rows, err := r.db.Query(ctx, `
		...
		RETURNING s.server_id::text, s.keep_count
	`)
	if err != nil {
	...
	for rows.Next() {
		var d dueSchedule
		if rows.Scan(&d.serverID, &d.keepCount) == nil {
			due = append(due, d)
		}
	}
	rows.Close()
	...
	for _, d := range due {
		...
		`, d.serverID).Scan(&d.nodeID); err != nil {
			continue
		}
...
func (r *Runner) runScheduledBackup(ctx context.Context, d dueSchedule) {
	...
	if r.remoteBackupsEnabled(ctx) {
		...
		_ = r.db.QueryRow(ctx, `
			INSERT INTO core.server_backups (server_id, status, filename, size_bytes, completed_at, source)
			VALUES ($1, 'completed', $2, $3, now(), 'schedule')
			RETURNING id::text
		`, d.serverID, name, size).Scan(&backupID)
		r.EnqueueBackupUpload(ctx, d.serverID, backupID, name, d.keepCount)
	}
