...
// emitWebhook ставит событие в очередь доставки арендатора.
//
// События server.status и node.offline были в списке подписки, но не
// отправлялись ниоткуда: статусы серверов и связь с нодой знает только реле,
// а очередь вебхуков наполнял один core-api. Доставку по-прежнему выполняет
// worker, здесь только запись в очередь.
