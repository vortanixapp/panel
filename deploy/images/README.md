...
PHP 7 и 8.0 для старых ядер pmmp больше не собирает — такие сборки берутся по
своей ссылке. Версию PHP для автоматической установки задаёт `/data/.vtx/php`
или переменная `PHP_VERSION`, ссылку на архив — `/data/.vtx/php_url` или
переменная `PHP_DOWNLOAD_URL`, файл ядра — `POCKETMINE_FILE`. По ссылке
принимается `tar.gz`, внутри которого лежит `bin/php`, `bin/<папка>/bin/php`
или та же раскладка во вложенном каталоге. Смена ссылки перекачивает PHP при
следующем запуске: рядом с версией в `/data/.vtx/pocketmine_php` хранятся путь
до бинаря и источник, откуда он приехал.

Оба файла пишет панель: в админке у игры PocketMine есть вкладка **PHP**, где
задаётся список версий, доступных клиенту, — встроенные `8.1`–`8.3` можно
выключить, а свои добавить строкой «версия + ссылка на сборку». Клиент
выбирает версию из этого списка на вкладке сервера. Для PHP 7 серверу
понадобится и своё ядро: последние ядра PocketMine на семёрке не работают. `server-port` и `server-portv6` (порт + 1)
