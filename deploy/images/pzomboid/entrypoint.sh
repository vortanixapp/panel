...
ADMIN_PASS_FILE="$DATA_DIR/.vtx/admin_password"
COMBINED_ARGS="$*"
if ! echo "$COMBINED_ARGS" | grep -qiE '(^| )-adminpassword( |$)'; then
  if [ ! -f "$ADMIN_PASS_FILE" ]; then
    mkdir -p "$DATA_DIR/.vtx" 2>/dev/null || true
    GENERATED_PASS="$(cat /dev/urandom 2>/dev/null | tr -dc 'A-Za-z0-9' | head -c 16 || true)"
    if [ "${#GENERATED_PASS}" -lt 8 ]; then
      GENERATED_PASS="PZ$(date +%s)Vx"
    fi
    printf '%s\n' "$GENERATED_PASS" > "$ADMIN_PASS_FILE" 2>/dev/null || true
    echo "[$GAME_KEY] Generated admin password and saved to .vtx/admin_password" >&2
  fi
  if [ -f "$ADMIN_PASS_FILE" ]; then
    ADMIN_PASS="$(head -n 1 "$ADMIN_PASS_FILE" 2>/dev/null | tr -d '\r\n')"
    if [ "$ADMIN_PASS" != "" ]; then
