...
COMBINED_ARGS="$*"
if ! echo "$COMBINED_ARGS" | grep -qiE '(^| )-nographics( |$)'; then
  set -- "$@" -nographics -batchmode
fi
if ! echo "$COMBINED_ARGS" | grep -qiE '(^| )-port( |$)'; then
  set -- "$@" -port "$GAME_PORT"
fi
if ! echo "$COMBINED_ARGS" | grep -qiE '(^| )-name( |$)'; then
  set -- "$@" -name "Valheim Server"
fi
if ! echo "$COMBINED_ARGS" | grep -qiE '(^| )-world( |$)'; then
  set -- "$@" -world "Dedicated"
fi
if ! echo "$COMBINED_ARGS" | grep -qiE '(^| )-password( |$)'; then
