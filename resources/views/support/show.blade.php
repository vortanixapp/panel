...
@section('content')
        ...
        <div class="w-full px-4 sm:px-6 lg:px-8">
            <div class="mb-6 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('support.index') }}" class="inline-flex h-10 items-center justify-center rounded-xl border border-white/10 bg-black/10 px-4 text-xs font-semibold text-slate-200 hover:bg-black/15 hover:text-white">Назад</a>
                    @if((string) $ticket->status === 'open')
                        <form method="POST" action="{{ route('support.close', $ticket) }}" onsubmit="return confirm('Закрыть тикет?');">
                            @csrf
            ...
            @if (session('success'))
                <div class="mb-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-200">{{ session('success') }}</div>
            @endif
            @if (session('error'))
                <div class="mb-4 rounded-2xl border border-rose-500/20 bg-rose-500/10 px-4 py-3 text-sm text-rose-200">{{ session('error') }}</div>
            @endif
            ...
            <div class="rounded-3xl border border-white/10 bg-[#242f3d] shadow-sm overflow-hidden">
                ...
                @if((string) $ticket->status === 'open')
                    <div class="border-t border-white/10 bg-black/10 p-5">
                        <form method="POST" action="{{ route('support.reply', $ticket) }}" class="grid gap-3">
                            @csrf
                            <div>
                                <textarea name="message" rows="4" required class="block w-full rounded-xl border border-white/10 bg-black/10 px-3 py-2.5 text-sm text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500" placeholder="Введите сообщение..."></textarea>
                                @error('message')
    ...
    <script>
        (function () {
            ...
            const endpoint = @json(route('support.messages', $ticket));
            ...
            function buildMessageNode(m) {
                ...
                who.textContent = m.is_admin ? 'Поддержка' : 'Вы';
