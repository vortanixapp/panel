    ...
    <section class="px-4 py-6 md:py-8">
        <div class="w-full max-w-none space-y-6">
            <div class="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
                <div>
                    <h1 class="text-xl md:text-2xl font-semibold tracking-tight text-slate-100">Редактирование тарифа</h1>
                    <p class="mt-1 text-sm text-slate-300/80">Обновите параметры тарифа: {{ $tariff->name }}.</p>
                </div>
            ...
            <form method="POST" action="{{ route('admin.tariffs.update', $tariff) }}" class="space-y-6 rounded-2xl border border-white/10 bg-[#242f3d] p-6 shadow-sm shadow-black/20" x-data="{ billingType: '{{ old('billing_type', $tariff->billing_type ?? 'resources') }}', cpuCores: {{ (int) old('cpu_cores', $tariff->cpu_cores) }} }">
                @csrf
                @method('PUT')
                        ...
                        <div class="grid gap-4">
                            <div class="space-y-1">
                                ...
                                >
                                    ...
                                    @foreach($locations as $location)
                                        <option value="{{ $location->id }}" {{ old('location_id', $tariff->location_id) == $location->id ? 'selected' : '' }}>
                                            {{ $location->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="space-y-1">
                                ...
                                >
                                    ...
                                    @foreach($games as $game)
                                        <option value="{{ $game->id }}" {{ old('game_id', $tariff->game_id) == $game->id ? 'selected' : '' }}>
                                            {{ $game->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="space-y-1">
                                ...
                                <select
                                    ...
                                    x-model="billingType"
                                >
                                    <option value="resources" {{ old('billing_type', $tariff->billing_type ?? 'resources') === 'resources' ? 'selected' : '' }}>Оплата за ресурсы</option>
                                    <option value="slots" {{ old('billing_type', $tariff->billing_type ?? '') === 'slots' ? 'selected' : '' }}>Оплата за слоты</option>
                                </select>
                                    ...
                                    @foreach([15, 30, 60, 180] as $days)
                                        <label class="inline-flex">
                                            <input
                                                ...
                                                name="rental_periods[]"
                                                value="{{ $days }}"
                                                {{ in_array($days, old('rental_periods', $tariff->rental_periods ?? [])) ? 'checked' : '' }}
                                                class="peer sr-only"
                                            >
                                    ...
                                    @foreach([15, 30, 60, 180] as $days)
                                        <label class="inline-flex">
                                            <input
                                                ...
                                                value="{{ $days }}"
                                                {{ in_array($days, old('renewal_periods', $tariff->renewal_periods ?? [])) ? 'checked' : '' }}
                                                class="peer sr-only"
                                            >
                                            <span class="inline-flex items-center justify-center rounded-md border border-white/10 bg-black/10 px-3 py-1.5 text-xs font-medium text-slate-200 shadow-sm hover:bg-black/15 peer-checked:border-sky-500 peer-checked:bg-sky-600 peer-checked:text-white">
                                                {{ $days }}
                            ...
                            <div class="grid gap-4 md:grid-cols-3">
                                <div class="space-y-1">
                                    ...
                                    <input
                                        ...
                                        step="0.01"
                                        min="0"
                                        value="{{ old('price_per_cpu_core', $tariff->price_per_cpu_core ?? 0) }}"
                                        class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
                                    >
                                </div>
                                ...
                                <div class="space-y-1">
                                    ...
                                    <input
                                        ...
                                        step="0.01"
                                        min="0"
                                        value="{{ old('price_per_ram_gb', $tariff->price_per_ram_gb ?? 0) }}"
                                        class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
                                    >
                                </div>
                                ...
                                <div class="space-y-1">
                                    ...
                                    <input
                                        ...
                                        step="0.01"
                                        min="0"
                                        value="{{ old('price_per_disk_gb', $tariff->price_per_disk_gb ?? 0) }}"
                                        class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
                                    >
                                </div>
                            </div>
                            ...
                            <div class="grid gap-4 md:grid-cols-3">
                                <div class="space-y-1">
                                    ...
                                    <input
                                        ...
                                        step="0.01"
                                        min="0"
                                        value="{{ old('price_per_slot', $tariff->price_per_slot) }}"
                                ...
                                <div class="space-y-1">
                                    ...
                                    <input
                                        ...
                                        type="number"
                                        min="1"
                                        value="{{ old('min_slots', $tariff->min_slots) }}"
                                        x-bind:required="billingType === 'slots'"
                                        class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
                                    >
                                ...
                                <div class="space-y-1">
                                    ...
                                    <input
                                        ...
                                        type="number"
                                        min="1"
                                        value="{{ old('max_slots', $tariff->max_slots) }}"
                                        x-bind:required="billingType === 'slots'"
                                        class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
                                    >
                        ...
                        <div class="grid gap-4">
                            <div class="space-y-1" x-show="billingType === 'resources'" x-cloak>
                                ...
                                <input
                                    ...
                                    step="0.01"
                                    min="0"
                                    value="{{ old('base_price_monthly', $tariff->base_price_monthly ?? 0) }}"
                            ...
                            <div class="grid gap-4 md:grid-cols-2">
                                <div class="space-y-1">
                                    ...
                                    <input
                                        ...
                                        type="number"
                                        min="0"
                                        value="{{ old('position', $tariff->position) }}"
                                        required
                                        class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
                                    >
                                    ...
                                    <div class="flex items-center justify-between gap-4">
                                        ...
                                        <label class="relative inline-flex shrink-0 cursor-pointer items-center">
                                            <input
                                                ...
                                                value="1"
                                                class="peer sr-only"
                                                {{ old('is_available', $tariff->is_available) ? 'checked' : '' }}
                                            >
                    ...
                    <div class="space-y-4 md:order-2">
                        ...
                        <div class="grid gap-4">
                            <div class="space-y-2">
                                <div class="flex items-center justify-between gap-4">
                                    ...
                                    <label class="relative inline-flex shrink-0 cursor-pointer items-center">
                                        <input
                                            ...
                                            value="1"
                                            class="peer sr-only"
                                            {{ old('allow_antiddos', $tariff->allow_antiddos) ? 'checked' : '' }}
                                        >
                                        <span class="h-5 w-9 rounded-full border border-white/10 bg-black/10 transition peer-checked:border-sky-500 peer-checked:bg-sky-600"></span>
                                        <span class="pointer-events-none absolute left-0.5 top-0.5 h-4 w-4 rounded-full bg-white/80 shadow transition peer-checked:translate-x-4"></span>
                                    </label>
                                ...
                                <div class="space-y-1">
                                    ...
                                    <input
                                        ...
                                        step="0.01"
                                        min="0"
                                        value="{{ old('antiddos_price', $tariff->antiddos_price ?? 0) }}"
                                        class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
                                    >
                                </div>
                            </div>
                            ...
                            <div class="space-y-1">
                                ...
                                >{{ old('discounts', $tariff->discounts ? json_encode($tariff->discounts) : '') }}</textarea>
                            ...
                            <div class="grid gap-4 md:grid-cols-3">
                                <div class="space-y-1">
                                    ...
                                    <input
                                        ...
                                        type="number"
                                        min="0"
                                        value="{{ old('cpu_min', $tariff->cpu_min) }}"
                                        class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
                                    >
                                </div>
                                <div class="space-y-1">
                                    ...
                                    <input
                                        ...
                                        type="number"
                                        min="0"
                                        value="{{ old('cpu_max', $tariff->cpu_max) }}"
                                        class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
                                    >
                                </div>
                                <div class="space-y-1">
                                    ...
                                    <input
                                        ...
                                        type="number"
                                        min="1"
                                        value="{{ old('cpu_step', $tariff->cpu_step) }}"
                                        class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
                                    >
                                </div>
                            </div>
                            ...
                            <div class="grid gap-4 md:grid-cols-3">
                                <div class="space-y-1">
                                    ...
                                    <input
                                        ...
                                        type="number"
                                        min="0"
                                        value="{{ old('ram_min', $tariff->ram_min) }}"
                                        class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
                                    >
                                </div>
                                <div class="space-y-1">
                                    ...
                                    <input
                                        ...
                                        type="number"
                                        min="0"
                                        value="{{ old('ram_max', $tariff->ram_max) }}"
                                        class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
                                    >
                                </div>
                                <div class="space-y-1">
                                    ...
                                    <input
                                        ...
                                        type="number"
                                        min="1"
                                        value="{{ old('ram_step', $tariff->ram_step) }}"
                                        class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
                                    >
                                </div>
                            </div>
                            ...
                            <div class="grid gap-4 md:grid-cols-3">
                                <div class="space-y-1">
                                    ...
                                    <input
                                        ...
                                        type="number"
                                        min="0"
                                        value="{{ old('disk_min', $tariff->disk_min) }}"
                                        class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
                                    >
                                </div>
                                <div class="space-y-1">
                                    ...
                                    <input
                                        ...
                                        type="number"
                                        min="0"
                                        value="{{ old('disk_max', $tariff->disk_max) }}"
                                        class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
                                    >
                                </div>
                                <div class="space-y-1">
                                    ...
                                    <input
                                        ...
                                        type="number"
                                        min="1"
                                        value="{{ old('disk_step', $tariff->disk_step) }}"
                                        class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
                                    >
                                </div>
                            </div>
                        ...
                        <div class="grid gap-4 md:grid-cols-2">
                            <div class="space-y-1">
                                ...
                                <input
                                    ...
                                    type="number"
                                    min="0"
                                    value="{{ old('cpu_cores', $tariff->cpu_cores) }}"
                            ...
                            <div class="space-y-1">
                                ...
                                <input
                                    ...
                                    min="2"
                                    max="262144"
                                    value="{{ old('cpu_shares', $tariff->cpu_shares) }}"
                        ...
                        <div class="grid gap-4 md:grid-cols-2">
                            <div class="space-y-1">
                                ...
                                <input
                                    ...
                                    type="number"
                                    min="1"
                                    value="{{ old('ram_gb', $tariff->ram_gb) }}"
                                    required
                                    class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
                                >
                            ...
                            <div class="space-y-1">
                                ...
                                <input
                                    ...
                                    type="number"
                                    min="1"
                                    value="{{ old('disk_gb', $tariff->disk_gb) }}"
                                    required
                                    class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
                                >
                ...
                <div class="flex items-center justify-end gap-3 pt-4">
                    <a href="{{ route('admin.tariffs.show', $tariff) }}" class="text-xs text-slate-300/80 hover:text-white">Отмена</a>
                    <button
                        type="submit"
