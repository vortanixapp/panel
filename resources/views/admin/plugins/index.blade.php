    ...
    <section class="px-4 py-6 md:py-8">
        <div class="w-full max-w-none space-y-6">
            ...
            <div class="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
                <div>
                    <h1 class="text-xl md:text-2xl font-semibold tracking-tight text-slate-100">Плагины</h1>
                    <p class="mt-1 text-sm text-slate-300/80">Каталог плагинов для установки на игровые серверы.</p>
                </div>
                <div class="flex items-center gap-3">
                    <a
                        href="{{ route('admin.plugins.create') }}"
            ...
            <div class="overflow-hidden rounded-2xl border border-white/10 bg-[#242f3d] shadow-sm text-sm">
                    ...
                    <table class="min-w-full divide-y divide-white/10">
                        <thead class="bg-black/10 text-[11px] uppercase tracking-wide text-slate-300/70">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-2 text-left">Путь установки</th>
                        ...
                        <tbody class="divide-y divide-white/10 text-[13px]">
                            @forelse ($plugins as $plugin)
                                <tr>
                                    <td class="px-4 py-2 align-top">
                                        <div class="font-medium text-slate-100">{{ $plugin->name }}</div>
                                    </td>
                                    <td class="px-4 py-2 align-top">
                                        <div class="text-slate-200">{{ $plugin->category ?: '—' }}</div>
                                    </td>
                                    <td class="px-4 py-2 align-top">
                                        <div class="text-slate-200">{{ $plugin->slug }}</div>
                                    </td>
                                    <td class="px-4 py-2 align-top">
                                        <div class="text-slate-200">{{ $plugin->version ?: '—' }}</div>
                                    </td>
                                    <td class="px-4 py-2 align-top">
                                        <div class="text-slate-200">{{ $plugin->archive_type }}</div>
                                    </td>
                                    <td class="px-4 py-2 align-top">
                                        <div class="text-slate-200">{{ $plugin->install_path }}</div>
                                    </td>
                                    <td class="px-4 py-2 align-top">
                                        @if($plugin->active)
                                    ...
                                    <td class="px-4 py-2 align-top">
                                        <div class="flex items-center gap-2">
                                            <a
                                                href="{{ route('admin.plugins.edit', $plugin) }}"
                                            ...
                                            <form
                                                method="POST"
                                                action="{{ route('admin.plugins.destroy', $plugin) }}"
                                                onsubmit="return confirm('Удалить плагин {{ $plugin->name }}?');"
                                            >
                                                @csrf
                                                @method('DELETE')
                                                <button
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="px-4 py-8 text-center text-[13px] text-slate-300/70">Плагины пока не добавлены.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                @if(method_exists($plugins, 'links'))
