...
@section('page_title', 'Редактировать плагин: ' . $plugin->name)
    ...
    <section class="px-4 py-6 md:py-8">
        <div class="w-full max-w-none space-y-6">
            <div class="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
                <div>
                    <h1 class="text-xl md:text-2xl font-semibold tracking-tight text-slate-100">Редактировать плагин</h1>
                    <p class="mt-1 text-sm text-slate-300/80">{{ $plugin->name }}</p>
                </div>
                <a
            ...
            <div class="rounded-2xl border border-white/10 bg-[#242f3d] p-6 shadow-sm shadow-black/20">
                ...
                <form method="POST" action="{{ route('admin.plugins.update', $plugin) }}" class="space-y-4" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')

                    @php
                        $faList = is_array($plugin->file_actions ?? null) ? (array) $plugin->file_actions : [];
                        if (count($faList) === 0) {
                            $faList = [['path' => '', 'lines' => []]];
                        }

                        $uaList = is_array($plugin->uninstall_actions ?? null) ? (array) $plugin->uninstall_actions : [];
                        if (count($uaList) === 0) {
                            $uaList = [['path' => '', 'lines' => []]];
                        }

                        $supported = is_array($plugin->supported_games ?? null) ? (array) $plugin->supported_games : [];
                        $supportedNorm = array_map(fn($v) => strtolower((string) $v), $supported);
                        $allGames = in_array('*', $supportedNorm, true) || count($supportedNorm) === 0;
                    @endphp

                    <div class="grid gap-4 md:grid-cols-2">
                        <div class="space-y-1">
                            <label for="name" class="text-xs font-medium text-slate-200">Название</label>
                            <input id="name" name="name" type="text" value="{{ old('name', $plugin->name) }}" required class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500">
                        </div>

                        <div class="space-y-1">
                            <label for="category" class="text-xs font-medium text-slate-200">Категория</label>
                            <input id="category" name="category" type="text" value="{{ old('category', $plugin->category) }}" class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500" placeholder="metamod / rehlds / admin / fun">
                        </div>

                        <div class="space-y-1">
                            <label for="slug" class="text-xs font-medium text-slate-200">Slug</label>
                            <input id="slug" name="slug" type="text" value="{{ old('slug', $plugin->slug) }}" required class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500">
                        </div>

                        <div class="space-y-1">
                            <label for="version" class="text-xs font-medium text-slate-200">Версия</label>
                            <input id="version" name="version" type="text" value="{{ old('version', $plugin->version) }}" class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500">
                        </div>
                        ...
                        <div class="space-y-1">
                            ...
                            <select id="archive_type" name="archive_type" required class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500">
                                @php $t = old('archive_type', $plugin->archive_type); @endphp
                        ...
                        <div class="space-y-1 md:col-span-2">
                            <label for="archive" class="text-xs font-medium text-slate-200">Новый архив (опционально)</label>
                            <input id="archive" name="archive" type="file" class="block w-full text-xs text-slate-200">
                            <div class="mt-1 text-[11px] text-slate-300/70">Текущий: {{ $plugin->archive_path }}</div>
                        </div>

                        <div class="space-y-1 md:col-span-2">
                            <label for="install_path" class="text-xs font-medium text-slate-200">Путь установки (относительно data_dir)</label>
                            <input id="install_path" name="install_path" type="text" value="{{ old('install_path', $plugin->install_path) }}" required class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500">
                        </div>
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <label class="inline-flex items-center gap-2 text-xs text-slate-200">
                                <input type="checkbox" name="all_games" value="1" {{ old('all_games', $allGames) ? 'checked' : '' }}>
                                <span>Все игры</span>
                            </label>
                            ...
                            <div class="grid gap-2 md:grid-cols-3">
                                @foreach(($games ?? []) as $g)
                                    ...
                                    @if($code !== '')
                                        <label class="inline-flex items-center gap-2 text-xs text-slate-200">
                                            <input type="checkbox" name="supported_games_codes[]" value="{{ $code }}" {{ in_array($code, old('supported_games_codes', $supportedNorm), true) ? 'checked' : '' }}>
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <div id="fileActionsWrap" class="space-y-3">
                                @foreach($faList as $idx => $fa)
                                    @php
                                        $p = is_array($fa) ? (string) ($fa['path'] ?? '') : '';
                                        $lines = is_array($fa) ? ($fa['lines'] ?? []) : [];
                                    ...
                                    <div class="rounded-xl border border-white/10 bg-black/10 p-3" data-file-action>
                                        ...
                                        <div class="grid gap-3 md:grid-cols-2">
                                            @php
                                                $aAction = is_array($fa) ? (string) ($fa['action'] ?? 'ensure_contains') : 'ensure_contains';
                                                $aCreate = is_array($fa) ? (bool) ($fa['create_if_missing'] ?? true) : true;
                                                $aPattern = is_array($fa) ? (string) ($fa['pattern'] ?? '') : '';
                                                $aReplacement = is_array($fa) ? (string) ($fa['replacement'] ?? '') : '';
                                            @endphp

                                            <div class="space-y-1">
                                                <label class="text-[11px] text-slate-300/80">Действие</label>
                                                @php $act = old('file_action_action.' . $idx, $aAction); @endphp
                                                <select name="file_action_action[]" class="fileActionAction block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm">
                                                    <option value="ensure_contains" @if($act==='ensure_contains') selected @endif>Добавить строки, если нет</option>
                                                    <option value="append_lines" @if($act==='append_lines') selected @endif>Дополнить в конец</option>
                                                    <option value="prepend_lines" @if($act==='prepend_lines') selected @endif>Вставить в начало</option>
                                                    <option value="remove_lines" @if($act==='remove_lines') selected @endif>Удалить строки</option>
                                                    <option value="write_file" @if($act==='write_file') selected @endif>Перезаписать файл</option>
                                                    <option value="replace_regex" @if($act==='replace_regex') selected @endif>Заменить по regex</option>
                                                </select>
                                            </div>

                                            <div class="space-y-1">
                                                <label class="text-[11px] text-slate-300/80">Путь к файлу</label>
                                                <input name="file_action_path[]" type="text" value="{{ old('file_action_path.' . $idx, $p) }}" class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm" placeholder="cstrike/server.cfg">
                                            </div>

                                            <div class="space-y-1 fileActionRegexFields md:col-span-2">
                                                <label class="text-[11px] text-slate-300/80">Regex pattern</label>
                                                <input name="file_action_pattern[]" type="text" value="{{ old('file_action_pattern.' . $idx, $aPattern) }}" class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm" placeholder="^sv_password.*$">
                                            </div>
                                            <div class="space-y-1 fileActionRegexFields md:col-span-2">
                                                <label class="text-[11px] text-slate-300/80">Regex replacement</label>
                                                <input name="file_action_replacement[]" type="text" value="{{ old('file_action_replacement.' . $idx, $aReplacement) }}" class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm" placeholder="sv_password \"123\"">
                                            </div>
                                            ...
                                            <div class="flex items-center gap-2 text-xs text-slate-200 fileActionCreateWrap md:col-span-2">
                                                ...
                                                <input type="checkbox" name="file_action_create_if_missing[]" value="1" {{ old('file_action_create_if_missing.' . $idx, $aCreate ? '1' : '0') ? 'checked' : '' }}>
                                                <span>Создать файл, если отсутствует</span>
                                            </div>

                                            <div class="space-y-1 md:col-span-2">
                                                <label class="text-[11px] text-slate-300/80">Строки</label>
                                                <textarea name="file_action_lines[]" rows="5" class="fileActionLines block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm" placeholder="sv_allowdownload \"1\"">{{ old('file_action_lines.' . $idx, $text) }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <div id="uninstallActionsWrap" class="space-y-3">
                                @foreach($uaList as $idx => $ua)
                                    @php
                                        $p2 = is_array($ua) ? (string) ($ua['path'] ?? '') : '';
                                        $lines2 = is_array($ua) ? ($ua['lines'] ?? []) : [];
                                        if (!is_array($lines2)) { $lines2 = []; }
                                        $text2 = implode("\n", array_map(fn($l) => rtrim((string)$l, "\r\n"), $lines2));

                                        $uAction = is_array($ua) ? (string) ($ua['action'] ?? 'ensure_contains') : 'ensure_contains';
                                        $uCreate = is_array($ua) ? (bool) ($ua['create_if_missing'] ?? true) : true;
                                        $uPattern = is_array($ua) ? (string) ($ua['pattern'] ?? '') : '';
                                        $uReplacement = is_array($ua) ? (string) ($ua['replacement'] ?? '') : '';
                                    @endphp
                                    <div class="rounded-xl border border-white/10 bg-black/10 p-3" data-uninstall-action>
                                        ...
                                        <div class="grid gap-3 md:grid-cols-2">
                                            <div class="space-y-1">
                                                <label class="text-[11px] text-slate-300/80">Действие</label>
                                                @php $actU = old('uninstall_action_action.' . $idx, $uAction); @endphp
                                                <select name="uninstall_action_action[]" class="uninstallActionAction block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm">
                                                    <option value="ensure_contains" @if($actU==='ensure_contains') selected @endif>Добавить строки, если нет</option>
                                                    <option value="append_lines" @if($actU==='append_lines') selected @endif>Дополнить в конец</option>
                                                    <option value="prepend_lines" @if($actU==='prepend_lines') selected @endif>Вставить в начало</option>
                                                    <option value="remove_lines" @if($actU==='remove_lines') selected @endif>Удалить строки</option>
                                                    <option value="write_file" @if($actU==='write_file') selected @endif>Перезаписать файл</option>
                                                    <option value="replace_regex" @if($actU==='replace_regex') selected @endif>Заменить по regex</option>
                                                </select>
                                            </div>

                                            <div class="space-y-1">
                                                <label class="text-[11px] text-slate-300/80">Путь к файлу</label>
                                                <input name="uninstall_action_path[]" type="text" value="{{ old('uninstall_action_path.' . $idx, $p2) }}" class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm" placeholder="cstrike/liblist.gam">
                                            </div>

                                            <div class="space-y-1 uninstallActionRegexFields md:col-span-2">
                                                <label class="text-[11px] text-slate-300/80">Regex pattern</label>
                                                <input name="uninstall_action_pattern[]" type="text" value="{{ old('uninstall_action_pattern.' . $idx, $uPattern) }}" class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm" placeholder="^gamedll_linux.*$">
                                            </div>
                                            <div class="space-y-1 uninstallActionRegexFields md:col-span-2">
                                                <label class="text-[11px] text-slate-300/80">Regex replacement</label>
                                                <input name="uninstall_action_replacement[]" type="text" value="{{ old('uninstall_action_replacement.' . $idx, $uReplacement) }}" class="block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm" placeholder="gamedll_linux \"dlls/cs_i386.so\"">
                                            </div>
                                            ...
                                            <div class="flex items-center gap-2 text-xs text-slate-200 uninstallActionCreateWrap md:col-span-2">
                                                ...
                                                <input type="checkbox" name="uninstall_action_create_if_missing[]" value="1" {{ old('uninstall_action_create_if_missing.' . $idx, $uCreate ? '1' : '0') ? 'checked' : '' }}>
                                                <span>Создать файл, если отсутствует</span>
                                            </div>

                                            <div class="space-y-1 md:col-span-2">
                                                <label class="text-[11px] text-slate-300/80">Строки</label>
                                                <textarea name="uninstall_action_lines[]" rows="5" class="uninstallActionLines block w-full rounded-md border border-white/10 bg-black/10 px-3 py-2 text-xs text-slate-100 shadow-sm">{{ old('uninstall_action_lines.' . $idx, $text2) }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                        ...
                        <div class="flex items-center gap-4 md:col-span-2">
                            <label class="inline-flex items-center gap-2 text-xs text-slate-200">
                                <input type="checkbox" name="restart_required" value="1" {{ old('restart_required', $plugin->restart_required) ? 'checked' : '' }}>
                                <span>Требуется рестарт</span>
                            </label>
                            <label class="inline-flex items-center gap-2 text-xs text-slate-200">
                                <input type="checkbox" name="active" value="1" {{ old('active', $plugin->active) ? 'checked' : '' }}>
                                <span>Активен</span>
                            </label>
                        </div>
                    </div>
