...
func RuntimeBuilds() []struct {
	Kind  string
	Image string
} {
	order := []Runtime{RuntimeSteam, RuntimeSrcds, RuntimeJava, RuntimeNative}
	out := make([]struct {
		Kind  string
		Image string
	}, 0, len(order))
	for _, r := range order {
		spec, ok := runtimeImages[r]
		if !ok {
			continue
		}
		image := spec.Image
		...
		out = append(out, struct {
			Kind  string
			Image string
		}{Kind: string(r), Image: withRegistry(image)})
	}
	return out
}
