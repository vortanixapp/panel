...
// Профили игр на движках GoldSrc и Source: CS 1.6, CS2, CSS, TF2, Garry's Mod.
//
// Настройки у них живут в server.cfg соответствующего мода, а булевы значения
// записываются единицей и нулём — за это отвечают поля True и False.

var mapNamePattern = regexp.MustCompile(`^[A-Za-z0-9_-]+$`)

// sourceCommonFields — то, что есть у любой игры на Source.
	...
	return []Field{
		{
			...
			// Имя подхватывается командой RCON без перезапуска, но только пока
			// сервер запущен; в конфиге оно всё равно нужно для следующего старта.
			AppliesLive: false,
		},
		{
...
func init() {
	// CS 1.6. Стартовая карта задаётся не конфигом, а аргументом +map: движок
	// читает её при запуске, а не из server.cfg.
	register(Profile{
		Key: "cs16",
		Files: []ConfigFile{
	...
	register(Profile{
		...
		Fields: append(append(sourceCommonFields("main"), sourceInternetFields("main")...), Field{
			...
			// Ноль здесь законное значение — «ботов нет». Прежние правила
			// проверяли границу как «не ноль», и нижней границы фактически не было.
