...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $games = DB::table('games')->whereIn('slug', ['css', 'cs2'])->select('id', 'slug')->get();
        ...
        foreach ($locations as $loc) {
            ...
            foreach (['css', 'cs2'] as $slug) {
                if (! isset($gameIdsBySlug[$slug])) {
                    continue;
                }

                $gameId = $gameIdsBySlug[$slug];
                $name = $slug === 'cs2' ? 'CS2 - Default' : 'CS:S - Default';
    ...
    {
        $games = DB::table('games')->whereIn('slug', ['css', 'cs2'])->select('id')->get();
