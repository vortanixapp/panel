...
// waker раздаёт сигнал пробуждения всем циклам сразу: подписка на NATS одна,
// а слушателей столько, сколько баз.
type waker struct {
	mu  sync.Mutex
	out []chan struct{}
}
...
// superviseTenants поднимает набор циклов на каждую базу арендатора и
// продолжает следить за реестром: база появляется в момент установки панели,
// то есть уже после старта воркера.
